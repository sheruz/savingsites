...
class CronController extends BaseController{
    ...
    public function dealsnotification(){
    	$date = date('Y-m-d');
		$day  = 1;
		$days = array(1=>'Monday', 2=>'Tuesday', 3=>'Wednesday',4=>'Thursday',5=>'Friday', 6=>'Saturday',7=>'Sunday');
		$weekday = $days[date('w')];
    	$Zonedashboard = new Zonedashboard();
   		$sql = "SELECT * FROM users_favorites_deals";
        $data = $this->CommonController->SelectRawquery($sql,'resultArray');
        if(count($data) > 0){
        	foreach ($data as $k => $v) {
				$explodedate = json_decode($v['day']);
				$salessql = "SELECT subdomain FROM sales_zone where id='".$v['zoneId']."'";
        		$zonedomain = $this->CommonController->SelectRawquery($salessql,'row');
        		$url = $zonedomain->subdomain.'.savingssites.com';
				if(count($explodedate) > 0){
					foreach ($explodedate as $k1 => $v1) {
						if($v1 == 9){break;}
						if($v1 == 8){
							$subCatIds = base64_encode($v['subcatId']);
							$Zonedashboard->sendCustomemail(213,$v['userId'],$subCatIds,$url);
						}
						if($v1==1 && $weekday == 'Monday'){
							$Zonedashboard->sendCustomemail(213,$v['userId'],$subCatIds,$url);	
						}
						if($v1==2 && $weekday == 'Tuesday'){
							$Zonedashboard->sendCustomemail(213,$v['userId'],$subCatIds,$url);
						}
						if($v1==3 && $weekday == 'Wednesday'){
							$Zonedashboard->sendCustomemail(213,$v['userId'],$subCatIds,$url);
						}
						if($v1==4 && $weekday == 'Thursday'){
							$Zonedashboard->sendCustomemail(213,$v['userId'],$subCatIds,$url);
						}
						if($v1==5 && $weekday == 'Friday'){
							$Zonedashboard->sendCustomemail(213,$v['userId'],$subCatIds,$url);
						}
						if($v1==6 && $weekday == 'Saturday'){
							$Zonedashboard->sendCustomemail(213,$v['userId'],$subCatIds,$url);
						}
						if($v1==7 && $weekday == 'Sunday'){
							$Zonedashboard->sendCustomemail(213,$v['userId'],$subCatIds,$url);
						}
					}
				}
