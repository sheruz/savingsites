...
function deleteorderonlineservice(){
   ...
   function deal_reschedule(){
	...
	public function insert_deals(){
		...
		$this->display = trim($params['live_status']);
		$this->banner = 'Yes';
...
# + businessdetail 
	...
	public function businessdetail($businessid=0,$fromzoneid=0){
		$this->CronController->rankbusiness($fromzoneid);
		$businessArr = $pbcashcert = $categories = $keyaws = $EmailformatingSaved = $subcatnew = $all_sponsored_business_details = $selected_display_categories = $display_categories =  $display_categories = $adsarr = $get_paypal_info = $approval_message = $adsarr = [];
	   	$login_type = $login_id = $refer_code = $user_id = '';
		$bid = 1;
		$user = $this->ionAuth->user()->row();
	   	$uid = $business_first_password_change=  0;
	   	if(!empty($user)){ 
	   		$uid = $user->id;
  	 	...
  	 	$pagesidebar = isset($_GET['page'])?$_GET['page']:'businessdetail';
		...
		$data['state_list']=$this->states->get_state_dropdown();
		$data['business_owner_details']=$this->users->get_user_details($userid);
		$data['business_sponsored_status']=$this->business_model->get_sponsor_business_status($businessid);
		$data['get_bus_state_phone'] = $this->users->get_bus_state_phone($businessid);
		$data['gallery_images'] = $this->business_model->get_gallery_images($businessid); 
		$data['postdata'] = $_POST;
...
# - RemoveBarter
	...
	*/
	...
	public function editdealdata(){
		...
		$auction_sql_setting =  'SELECT ip.* , a.*,ip.deal_product_id,ip.product_name,  a.page_title ,a.auction_type,a.status,a.display,a.start_date,a.last_update FROM tbl_deals a LEFT JOIN tbl_deals_products ip ON ip.deal_product_id=a.product_id WHERE a.product_id='.$_REQUEST['productid'].'   order by a.start_date ASC';
   	    $auction_result_setting = $this->CommonController->SelectRawquery($auction_sql_setting,'resultArray');
