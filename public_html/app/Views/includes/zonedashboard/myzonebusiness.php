      ...
      <table class="display responsive-table datatable-all" cellspacing="0" width="100%" id="myzonebusinesstable">
        ...
        <tbody id="myzonebusinesdata">
          <?php foreach ($businessArr as $bk => $bv) { ?>
            ...
            <td><?= $bv->phone; ?></td>
            <td><?= $bv->zip_code; ?></td>
            <td><a href="<?= base_url();?>/Zonedashboard/zonedetail/<?= $zone_id; ?>?page=viewads&busid=<?= $bv->id; ?>" class="tab_icon green" target="_blank"><b> AD</b></a>
                <a href="<?= base_url();?>/business/businessdetail/<?= $bv->id; ?>/<?= $zone_id; ?>" class="tab_icon" target="_blank"><i class="fa fa-id-card" aria-hidden="true"></i></a> 
                <a href="javascript:void(0)" class="tab_icon red"><i busid="<?= $bv->id; ?>" class="fa fa-trash deletebuisness" aria-hidden="true"></i></a>
            </td>
