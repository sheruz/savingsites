...
$(document).ready(function () { 
	...
	$('#phone_broadcasting_sites').addClass('active');
	
});
$(function(){
	if($('#show_phone_link').val() != '' ){
		$('#show_phonesites_link').hide();
		$('#message').show();
	}
});
...
function save_phone_info(){
	...
	if(authenticate=='0'){
		...
		if($("#phone_link").val() == '' && $("#phone_description").val() == ''){
			alert('Please enter the phone broadcasting information.'); return false;
		}else if($("#phone_link").val() == ''){
			alert('Please enter the Phone Broadcasting Link'); return false;
		}
		else if($("#phone_description").val() == ''){
			alert('Please enter the Phone Broadcasting description'); return false;
		} 
		var dataToUse = {
			"zoneid":$("#zoneid").val(),
		...
		PageMethod("<?=base_url('Zonedashboard/save_phone_link')?>", "Processing...<br/>This may take a minute.", dataToUse, phonebroadcastingSuccess, null);
	 }
}
 function phonebroadcastingSuccess(result){//alert(JSON.stringify(result));
	$.unblockUI();
	$("#phone_link").val("");
	$("#phone_description").val(""); 
	$('html,body').animate({scrollTop:0},"slow");
	//$("#msg").html('<h4 style="color:#090">Coupon created successfully</h4>').show();
	 setTimeout(function(){$("#msg").hide('slow');},3000);
	 var zoneid =$('#zoneid').val();
	 window.location.href = '<?=base_url('Zonedashboard/phone_broadcasting_view')?>' + '/'+ zoneid;
}
...
<input type="hidden" name="show_coupon1" id="show_phone_link" value="<?=$create_list['0']['phone_broadcasting'] ?>" />
<input type="hidden" name="zoneid" id="zoneid" value="<?=$common['zoneid']?>" />
<input type="hidden" name="show_phonesites_link" id="show_coup" value="<?=$create_list['0']['id']?>" />
	...
	<div id="container_tab_content" class="container_tab_content">
       ...
       <div id="show_phonesites_link">
            ...
            <p>
            	...
            	<div id="new_category">
                   ...
                   <label for="phone_link"  style="margin-top:0px;" class="fleft w_150">Enter Phone Broadcasting Link</label>
                      <input type="text" id="phone_link" name="phone_link" placeholder="Enter # (e.g. http://savingssites.com)"   class="w_300 coupon"/>
                    ...
                    <p>
                    <p id="result"></p>
                        <button class="m_left_150" onclick="save_phone_info()"  id="button">Save</button>
        ...
        <div id="message" style="display:none" >
                <h2 style="color:#090" align="center">Phone broadcasting  already exist in the system. Either delete and create new or update the existing.</h2>
         </div>
         <div id="message1" ></div>
        </div>
    	</div>
