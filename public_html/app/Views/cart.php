...
Savings Sites Business Search Cart
<?=$this->endSection()?>
...
<div id="cart_page" class="card">
            <div class="row">
                <div class="col-md-8 cart">
                    ...
                    foreach ($cartdata as $k => $v) {
                        ...
                        echo '<div class="row border-top border-bottom">
                                <div class="row main align-items-center">
                                    <div class="col-2"><img class="img-fluid" src="'.base_url().'/assets/SavingsUpload/Business/'.$busid.'/'.$v['options']['cardimg'].'"></div>
                                    <div class="col">
                    ...
                    ?> 
                        <div class="back-to-shop"><a href="<?= base_url();?>/businessSearch/search/<?= $zone_id ?>">&leftarrow;</a><span class="text-muted">Back to shop</span></div>
                </div>
                <div class="col-md-4 summary cart0">
                    ...
                    <div class="row" style="padding: 2vh 0;">
                        ...
                        <div class="row" style="border-top: 1px solid rgba(0,0,0,.1); padding: 2vh 0;">
                            ...
                            <div class="col text-right">$<?= number_format($credit_to_pays+$finalprice,"2",".",""); 
                            ?></div>
                        </div>  
                        <?php } ?> 
                    ...
                    <?php if (!empty($user_id)) { 
                        ...
                        if($get_paypal_info['paypal_url'] != ''){ 
                            ...
                            <form class="<?= $hide; ?>" name="_xclick " action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
                                <input type="hidden" name="cmd" value="_xclick">
                                <input type="hidden" name="business" value="<?php echo $get_paypal_info['paypal_url'] ?>">
                                <input type="hidden" name="currency_code" value="USD">
                                
                                <input type="hidden" name="return" value="<?= base_url();?>/thankyou/<?= $zoneid ?>?UserId=<?= $user_id ?>&Amount=<?php echo  $finalprice; ?>&creditStatus=<?php echo $creditStatus ?>&AucId=<?php echo $deaid;  ?>&token=<?php echo time(); ?>&busid=<?php echo $busid;  ?>&zoneid=<?php echo $zoneid;  ?>&usedcert=<?php echo $used_certificate;  ?>&businessim=<?= $businessim;?>&dealidim=<?= $dealidim;?>&amountim=<?= $amountim;?>&subtotalim=<?= $subtotalim;?>&disim=<?= $disim;?>&creditim=<?= $creditim;?>&disamountim=<?= $disamountim;?>&user_type="> 
