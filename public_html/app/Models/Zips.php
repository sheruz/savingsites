...
t1.zip = t2.zip where t1.uid = ?";
	...
	public function zip_to_zone($zip){		
		...
		if($newzip== false){
			...
			$sql="SELECT b.seo_zone_name,b.id,b.name, b.subdomainZone FROM zip_code_zone a,sales_zone b WHERE a.zone_id=b.id AND a.zone_id=".$zip;
			$query = $this->db->query($sql);
			$count_query= $query->getNumRows();
 			...
 			if($count_query>0){
				return $query->getResultArray();
		 		...
		 		$zipsql="SELECT b.seo_zone_name,b.id,b.name, b.subdomainZone FROM zip_code_zone a,sales_zone b WHERE a.zone_id=b.id AND a.zip_code=".$zip;
