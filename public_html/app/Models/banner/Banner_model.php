...
function save_banner($image_name='',$zone_id=0,$status=1,$description='',$order=0){
	...
	public function active_banner_desktopmobile($zone_id=0,$from='',$viewable_at = '',$device_type = ''){
		if($from == 'directory'){
			$limit= ' limit 1';
		}else{
			$limit = '';
	...
	public function checkZonelogo($zone_id){
		$query = "SELECT *  FROM sales_zone where id = ".$zone_id."";
		$row = $this->CommonController->SelectRawquery($query,'resultArray');
