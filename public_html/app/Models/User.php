...
class User extends CI_Model
{
	...
	##################################################
	// for update profile
	public function update_profile($data_arr=''){ //var_dump($data_arr); exit;
		if($data_arr!=''){
			$upsdata = array('email' => $data_arr['email'],'first_name' => $data_arr['firstname'],'last_name' => $data_arr['lastname'],'phone' => $data_arr['phone'],'Address' => $data_arr['address'],'City' => $data_arr['city'],'State_Code' => $data_arr['state'],'Zip' => $data_arr['zip']);
			
			/*if($data_arr['type']==3){ // for organization
				$sql="select userid from organization where id=".$data_arr['id'];
			}
			
			$query = $this->db->query($sql);
			$result = $query->result_array();*/
			$this->db->where('id', $data_arr['userid']);
        	$this->db->update('users', $upsdata);
		}
	}
    ...
    function check_username($username=false){ //var_dump($username);
	...
	function check_email($email=false){ //var_dump($username);
