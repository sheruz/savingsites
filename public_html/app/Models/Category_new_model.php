...
// add category, first subcategory in tha database
	...
	public function get_products_details($zoneid=0,$id=0){
	    $wh_cond = $subcat_id = '';
		$arr= array();
		if($zoneid!=0){
			if($id==1){
			...
			if($wh_in_arr != ''){
				...
				if($_x['child_type']=='y'){
					...
					$sql="SELECT a.id as subcat_id,a.name as subcat_name,b.* FROM category_subcategory_new as a,category_sub_subcategory_new as b, subcategory_display as c WHERE a.id=b.parent_id and b.id=c.subcatid and a.catid=c.catid and b.parent_type='s' and a.catid=".$_x['id']." and c.zoneid=".$zoneid.$wh_cond;
					$result = $this->CommonController->SelectRawquery($sql,'resultArray');
					$i=0; 
					foreach($result as $k=>$subcat){
