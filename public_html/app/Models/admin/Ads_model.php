...
### Athena eSolutions start
    ...
    public function get_ads_for_business_new($id='',$uid='',$lowerlimit='',$upperlimit='',$approval='',$busname='',$insert_via_csv = 0){
        $limit_where="";
        $i=0;       
        if($lowerlimit!='' && $upperlimit!=''){    
            $limit_where=" limit ".$lowerlimit.",".$upperlimit;
...
motto,c.israted,f.street_address_1 AS bs_streetaddress1,f.street_address_2 AS bs_streetaddress2,f.city AS bs_city,f.state AS bs_state,f.zip_code AS bs_zipcode,f.phone AS bs_phone,e.stickyad,c.websitevisibility,c.emailvisibility,i.job,j.barter,k.approval,m.rate,f.latitude,f.longitude,c.name as businessname,n.name as zone_name
    ...
    $businessArr = $addressArr = $business_owner_idArr = $usersArr = $finalArr = $adsArr = $addressDataArr = $business_sponsor_orderArr = $business_sponsorArr = $business_sponsorcatArr = $busimageArr = $usernameArr = $usertblArr = $usertbldealsArr = $usertbldealsprodcutsArr =$atozone = $favArr = [];
     $userArr1 = $tblmember1 = $tbldeals1 = $tblproduct1 = $AdidArr = $businessArrcat = $d = $sortArr = $InArr = $OutArr = $datainsertArr = $azArr = [];
    $totalcount = 0;
    ...
    if($page_dest == 'show_ads_specific_category' && $charval == ''){
        ...
        //     foreach ($inhdata as $key => $v) { $InArr1[] = $v->businessid; }
        //     if($cat_id > 0){
        //         if(count($InArr1) > 0){
        //             $inexplodenew1 = implode(',',$InArr1);
        //             $sql="SELECT `a`.business_id FROM `ads` as `a` JOIN `ad_category_subcategory` as `b` ON `a`.`id`=`b`.`adid` WHERE `b`.`zoneid` = ".$zone_id." AND `b`.`catid` = ".$cat_id." AND `a`.`business_id` IN (".$inexplodenew1.") ";
        //             $inhdata1 = $this->CommonController->SelectRawquery($sql, 'result');
        //             foreach ($inhdata1 as $key => $v) { $InArr[] = $v->business_id; }    
        //         }
        //     }
        ...
        //     foreach ($data as $k1 => $v1) { $OutArr1[] = $v1->businessid;}
        //     if($cat_id > 0){
        //         if(count($OutArr1) > 0){
        //             $inexplodenew2 = implode(',',$OutArr1);
        //             $sql="SELECT `a`.business_id FROM `ads` as `a` JOIN `ad_category_subcategory` as `b` ON `a`.`id`=`b`.`adid` WHERE `b`.`zoneid` = ".$zone_id." AND `b`.`catid` = ".$cat_id." AND `a`.`business_id` IN (".$inexplodenew2.") ";
        //             $outhdata2 = $this->CommonController->SelectRawquery($sql, 'result');
        //             foreach ($outhdata2 as $k1 => $v1) { $OutArr[] = $v1->business_id;}    
        //         }
        //     }
        //     /*deal not available date array*/
    ...
    if($charval == 'redemption_high' || $charval == 'redemption_low' || $charval == 'deal_price_high' || $charval == 'deal_price_low' || $charval == 'high to low' || $charval == 'low to high' || $charval == 'a-z' || $charval == 'z-a' || $charval == 'deal_left_high' || $charval == 'deal_left_low'){
        ...
        if($charval == 'z-a' || $charval == 'a-z'){
            ...
            $sql="SELECT `a`.*, `e`.`current_price`, `e`.`start_date`, `e`.`end_date` FROM `ads_setting_preferences` as `a` JOIN `business` as `b` ON `a`.`businessid`=`b`.`id` JOIN `users` as `c` ON `b`.`business_owner_id`=`c`.`id` JOIN `tbl_member` as `d` ON `c`.`username`=`d`.`user_name` JOIN `tbl_deals` as `e` ON `d`.`user_id`=`e`.`user_id` JOIN `tbl_deals_products` as `f` ON `d`.`user_id`=`f`.`user_id` WHERE `a`.`settingszoneid` = ".$zone_id." AND `a`.`approval` NOT IN(-1, -2, 3, -3)  and ('".date('Y-m-d')."' BETWEEN e.start_date AND DATE_FORMAT(e.end_date, '%Y-%m-%d')) ORDER BY ".$order_by." LIMIT ".$lowerlimit.", ".$upperlimit."";
        }
        $data = $this->CommonController->SelectRawquery($sql,'result');
    }
    ...
    if($page_dest == 'show_all_offers'){
        if($charval == ''){
            ...
            if($searchbutton == 'true'){
                ...
                $sql="SELECT `a`.*,`b`.name FROM `ads_setting_preferences` as `a` JOIN `business` as `b` ON `a`.`businessid`=`b`.`id` WHERE `b`.`name` LIKE '".trim($charval)."%' AND `a`.`settingszoneid` = ".$zone_id." LIMIT ".$lowerlimit.", ".$upperlimit."";
            }
            $data = $this->CommonController->SelectRawquery($sql,'result');
        }
    }
    ...
    if($page_dest == 'show_ads_specific_sub_category' || $sub_cat_id > 0){
        ...
        if(!in_array($sub_cat_id, $foodtypeArr)){
            $sql="SELECT business.*,business.id as businessid FROM business  
                ...
                WHERE business_sponsor_order_cat.zoneid=".$zone_id." and business_sponsor_order_cat.subcatid=".$sub_cat_id." GROUP BY business.id   ORDER BY business_sponsor_order_cat.display_order ASC  ";
            $data = $this->CommonController->SelectRawquery($sql,'result');   
        }else{
            $sql="SELECT * , business.*,business.id as businessid FROM business_sponsor_order 
                ...
                WHERE business_sponsor.zone_id=".$zone_id." and business_sponsor_order_cat.subcatid=".$sub_cat_id." GROUP BY business.id   ORDER BY business_sponsor_order_cat.display_order ASC  ";
            $data = $this->CommonController->SelectRawquery($sql,'result');
        }
        if(trim($day) == '01'){
        ...
        // $tableArr[] = ['table' => 'business as b','link' => 'a.businessid=b.id','type' =>'left'];
        // $sortbusiness = $this->CommonController->SelectJoinMulti('ads_setting_preferences as a', $tableArr, ['a.settingszoneid'=> $zone_id],['b.id' => $businessArrcat], ['b.id'=> 'ASC'],'*','','','');
    ...
    if($page_dest == 'show_data_miles'){
        if($user_id == 0){
            ...
            $city    = $json['city'];
            $url = "https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($city)."&sensor=false&key=AIzaSyCvpVpr64JO4z3XY-sMoe2nX4wsKRxI-Zw";
            $result_string = file_get_contents($url);
            $result = json_decode($result_string, true);
            $residentlat = $result['results'][0]['geometry']['location']['lat'];
            $residentlng = $result['results'][0]['geometry']['location']['lng'];    
        }else if($user_id > 0){
            $where = ['a.id'=> $user_id];
            ...
            $url = "https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($postalcode)."&sensor=false&key=AIzaSyCvpVpr64JO4z3XY-sMoe2nX4wsKRxI-Zw";
            $result_string = file_get_contents($url);
            $result = json_decode($result_string, true);
            $residentlat = $result['results'][0]['geometry']['location']['lat'];
            $residentlng = $result['results'][0]['geometry']['location']['lng'];   
        ...
        $sql="SELECT b.id as businessid,b.name,c.zip_code,c.id as addressid,c.latitude,c.longitude from ads_setting_preferences as a INNER JOIN business as b ON a.businessid=b.id INNER JOIN address as c ON b.addressid=c.id WHERE a.settingszoneid=".$zone_id." AND a.approval NOT IN (-1, -2, 3, -3)";
        $businessazarr = $this->CommonController->SelectRawquery($sql,'result');
        if($type == 'total'){
...
public function get_ads_for_all_athena_latest_working_anishtotal($zone_id=0,$lowerlimit=0,$upperlimit=0,$search_value='',$cat_id=0,$sub_cat_id=0,$user_id=0,$buseness_id=0,$ad_id=0,$charval='',$deal_title_ad_id=0,$deal_title_type='',$deal_title='',$food_deliver=0, $subcatparentid=0 , $multiplesubcatid = 0, $business_sponsor = 0,$page=0,$page_dest = 0){
    ...
    //     $joinArr[] = ['table' => 'ads as b','link' => 'a.business_id=b.business_id','type' => 'left']; 
    //     $joinArr[] = ['table' => 'ad_category_subcategory as c','link' => 'b.id=c.adid','type' => 'left'];
    //     $data = $this->CommonController->SelectJoinMulti('business_sponsor_order as a', $joinArr,$where,[], $orderby,'a.business_id as businessid','','','count');
    // }
...
# - webinar_information --> get the webinar details via business table using the ad id form the webinar information table
    ...
    public function show_dealsnew($adid,$zoneId,$filter='',$username = '', $busid = 0, $user_id = 0,$compname = '',$deal_id = 0){
        if($user_id != ''){
            $current_date = date('Y-m-d');
            $peekaboo_popup_sql = "SELECT b.*, d.product_name, d.nobypass,d.consolation_price,d.cert_accept,d.description,d.publisher_fee,d.card_img  , d.numberofconsolation FROM tbl_deals b
