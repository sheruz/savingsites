...
jQuery(document).ready(function () {RDMobilemenu_autoinit('#panelsMenu ul');});
...
$(".mobilePanel a.logo").on('click', function(e) {
        e.preventDefault();
        var type = $(this).data('type');  
        $(".mobilePanel a.logo").removeClass('active');      
        if('ss' == type){
            $("#area-business5").hide('fast');
            if($("#area-category").is(':visible')){
                $("#area-category").hide('fast');
            }else {
	...
	$(".show-pboopanel-xs").click(function(){
		$(".pboopanel-xs").removeClass("hidden-xs");
	});
});