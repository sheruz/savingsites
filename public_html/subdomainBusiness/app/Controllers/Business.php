...
class Business extends BaseController{
 ...
 function menu_generator($zoneid=0){  
	...
	public function businessdetail($busname){ 

		$refdealuser = isset($_GET['userid'])?$_GET['userid']:''; 
		$zoneid = $this->CommonController->redirectToBusinessdetail();
		$qry = "select * from business where slugname='".$busname."'";
 		$busslug = $this->CommonController->SelectRawquery($qry, 'row');
 		$bussid = isset($busslug->id)?$busslug->id:0;
 		
		$referdealid = 0;
		if($refdealuser > 0){
			$qry = "select * from user_refer_meta where from_user='".$refdealuser."'";
		...
		$session_session_normal_user_in_zone = $session_session_normal_user_type = $session_type = $session_type_id= $email = $firstName = $lastName = $businessUser = $session_usertype = $refer_code = $orderonlineservice = $anewid = $busimage = $busimagetype = $busimagepath = '';
		$countTotalImage = 0;
		$allBusimage = $addlist = [];
    	$date = date('Y-m-d');
    	...
    	if($this->ionAuth->loggedIn()){ 
			...
			if(isset($_GET['userid']) && $_GET['userid'] > 0 && $auser->id != $_GET['userid']){
		...
		if($this->CommonController->getSession('session_normal_user_in_zone') !=''){
		 ...
		 //$bussid = isset($_GET['bussid'])?$_GET['bussid']:'';
		// echo"<pre>";print_r($bussid);
		// die('here');
		...
		$auction_result_setting = [];
		if($peekaboo_type != '' && $peekaboo_id != ''){
		...
		$zone= $this->Zone_model->get_zone($zoneid);
	   	...
	   	if(isset($_GET['adid']) && $_GET['adid'] > 0){
			...
			if($dealimage != ''){
				$busimagepath = base_url().'/assets/SavingsUpload/Business/'.$bussid.'/'.$addlist[0]['card_img'].'';
				$busimagetype = $ext = pathinfo($addlist[0]['card_img'], PATHINFO_EXTENSION);
				$res = $this->resize_image($busimagepath, 386, 395,$addlist[0]['card_img'],$ext,$bussid);
				...
				if($addtextarr != ''){
					...
					$busimagepath = base_url().'/assets/SavingsUpload/Business/'.$bussid.'/'.$imagenew.'';	
					$busimagetype = $ext = pathinfo($addtextarr->adtext, PATHINFO_EXTENSION);
					$res = $this->resize_image($busimagepath, 386, 395,$imagenew,$ext,$bussid);
					$busimage = base_url().'/assets/SavingsUpload/Business/'.$bussid.'/'.$imagenew.'';
				}else{
					$busimagetype = 'png';
					$busimagepath = base_url().'/assets/SavingsUpload/images/new-savingssites3.png';
					$res = $this->resize_image('https://savingssites.com/assets/SavingsUpload/images/new-savingssites3.png', 386, 395,$imagenew,'png',$bussid);
					$busimage = 'https://savingssites.com/assets/SavingsUpload/images/new-savingssites3.png';
				}
			}
		}
		
		$sql="SELECT * FROM `res_favfood_audio` WHERE `day`='".$currentday."' AND business_id='".$bussid."'";
    	$audioArr = $this->CommonController->SelectRawquery($sql,'row');
    	$busaudio = isset($audioArr->audio)?$audioArr->audio:'';
    	$buspdf = isset($audioArr->pdf)?$audioArr->pdf:'';
    	if($busaudio != ''){
    		$audioexists = 1;
    		$busaudio1 = base_url().'/assets/SavingsUpload/Audio/'.$bussid.'/'.$busaudio;
    	}else{
    		$audioexists = 0;
    		$busaudio1 = '';
    	}
    	if($buspdf != ''){
    		$buspdf1 = base_url().'/assets/SavingsUpload/PDF/'.$bussid.'/'.$buspdf;
    	}else{
    		$buspdf1 = '';
    	} 
		return view('businessSearchdetail',array('zoneid'=>$zoneid,'zone_id'=>$zoneid,'user_id' =>$userid,'userid' =>$userid,'busid'=>$bussid,'email'=>$email,'firstName'=>$firstName,'lastName'=>$lastName,'businessUser'=>$businessUser,'session_usertype'=> $session_usertype,'session_session_normal_user_in_zone'=>$session_session_normal_user_in_zone,'session_session_normal_user_type'=> $session_session_normal_user_type,'session_type'=>$session_type,'session_type_id'=> $session_type_id,'busdata'=>$busdata,'zone_logo'=> $zone_logo,'zone_name'=> $zone_name,'adsdata'=> $adsdata,'orderonlineservice'=>$orderonlineservice,'get_paypal_info'=>$get_paypal_info,'peekaboocredits'=> $peekaboocredits,'countTotalImage'=> $countTotalImage,'busimages'=> $busimages,'deals'=> $deals,'auction_result_setting'=> $auction_result_setting,'theme'=> $theme,'refer_code'=> $refer_code,'zone_owner'=> $zone_owner,'business_theme'=> $business_theme,'business_page'=> $business_page,'business_id'=> $bussid,'hidefooter' =>'hide','showfooter'=>'','adsdataid'=> $adsdataid,'adsdatatitle'=> $adsdatatitle,'bnamenew'=> $zone_name,'adid'=> $anewid,'addlist'=> $addlist,'busimage'=> $busimage,'busimagetype'=> $busimagetype,'busaudio1'=> $busaudio1,'audioexists'=> $audioexists,'buspdf1'=> $buspdf1,'refdealuser'=> $refdealuser,'referdealid'=> $referdealid));
	}
	
	public function resize_image($file, $width, $height,$imgename,$ext,$bussid) {
		if($imgename == ''){
			$filename = 'https://savingssites.com/assets/SavingsUpload/images/new-savingssites3.png';
			$ext = 'png';
			$imgename = 'new-savingssites3.png';
		}else{
			$filename = $file;
		}
		$locationoriginal = "./assets/SavingsUpload/Business/".$bussid."/";
        ...
        if($ext == 'png'){
			imagepng($dst, $locationoriginal.$imgename);
        }else if($ext == 'webp'){
			imagewebp($dst, $locationoriginal.$imgename);
        }else{
			imagejpeg($dst, $locationoriginal.$imgename);
		}
    }
	...
	public function allpeekabooId($userID){
		...
		$auction_result_setting = $this->CommonController->SelectRawquery($auction_sql_setting, 'resultArray');
		//echo"<pre>";print_r($auction_result_setting);die;
		if($auction_result_setting!=""){
