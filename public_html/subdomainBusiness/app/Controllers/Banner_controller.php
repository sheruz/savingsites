...
/* 	function save_banner_image($zone_id){
	...
	public function save_zonelogo(){
		...
		$zonesubuser = isset($_REQUEST['subuser'])?$_REQUEST['subuser']:'';
		...
		if($zonesubuser != ''){
			$this->CommonController->InsertSubUserData('subuserlogs',$zonesubuser,'Save Zone logo',serialize(['image'=>$imagename,'zone'=>$zone]));
		}
	...
	public function save_buisnesslogo(){
		...
		if($zonesubuser != ''){
			$this->CommonController->InsertSubUserData('subuserlogs',$zonesubuser,'Business Logo',serialize(['logo' => $imagename,'id' => $busid]));
		}
	    echo json_encode(['type' => 'success','msg' => 'Logo Updated Successfully', 'data' => $imagename]);
	}
