...
class Auth extends BaseController
{
	...
	public function businessdashboard_update(){
        ...
        if($zonesubuser != ''){
			$this->CommonController->InsertSubUserData('subuserlogs',$zonesubuser,'Update Business Detail',serialize($business_data));
		}
	...
	public function profile_update(){
		...
		$userorgquery="SELECT a.phone,b.group_id FROM users as a INNER JOIN users_groups as b ON a.id=b.user_id WHERE a.phone = '".$name."' AND b.group_id=5";
		...
		$userorgnoquery="SELECT a.phone,b.group_id FROM users as a INNER JOIN users_groups as b ON a.id=b.user_id WHERE a.email = '".$uemail."' AND b.group_id=5";
        ...
        $additional_data = array('first_name' => $fname,'last_name' => $lname,'company' =>$company_name,'phone' => $phone,'Address' => $address,'City' => $city,'State_Code' => $state,'Zip' => $zip,'Zone_ID' => $selZone,'alternate_phone' => $alternate_phone,'gender' => $gender);
        ...
        if($id != '' && $zone==''){
        	...
        	//print_r($data_peekaboo);die('data');
    	...
    	if(!$id){            
            ...
            <hr></font>" ;
            	...
            	$message_body= '<body><div style="color:#000;background: #fff;width: 70%;margin: 50px auto;padding: 11px;border: 1px solid #ccc;font-size: 14px;line-height: 26px;box-shadow: 0px 1px 11px rgb(111 110 122 / 52%);"><div style="text-align: center;background: #f2f2f2;padding: 17px;margin-bottom: 20px"><img style="width: 240px;" src="https://cdn.savingssites.com/logo-green.png"/></div>
                 ...
                 <div style="background-color:#FFF; margin-top:10px; margin-bottom:10px; min-height:300px; padding:15px;">
                    <h2 style="text-align:left; color:#333;">Dear'."  ".$fname.', 
                    </h2><h2 style="text-align:left; display:block; color:#333;"> Thank you for registering with SavingsSites.</h2><h2 style="text-align:left; display:block; color:#333; font-size: 18.0pt;">The link below will take you to the directory homepage where you can login to your business dashboard to create your<br/> FREE Ad and INCREASE SALES!</h2>
                    <h3 style="text-align:left; display:block; color:#333;"><a href="'.$link.'">'.$link.'</a></h3>
                    <h3 style="text-align:left; display:block; color:#333;">Best Regards,</h3>
                    <h3 style="text-align:left; display:block; color:#333;">Your Friends at Savings Sites</h3>
                    </div>
                    <div style="background-color:#fff; height:60px;"></div></body>';
        	...
        	$this->CommonController->SendMail('',$uemail,$template_subject,$message_body);
			...
			if($selZone!=''){
                ...
                $message = '<div class="succ_messs_form">
                    ...
                    <div style="background-color:#fff; height:115px;text-align: center;"><img src="https://cdn.savingssites.com/logo-green.png" style="padding:14px 0px;" alt="logo"/></div>
            		...
            		$this->CommonController->SendMail('',$ZO_email,$template_subject,$message_body);
    ...
    public function sponsor_profile_update(){
    	$ZO_seo_name = $redirection_link = '';
        $account_type = 17;
        $accountGroups = [];
        $accountGroups[] = $account_type;
        ...
        $postalcode = (!empty($_REQUEST['postalcode']))? $_REQUEST['postalcode'] : "";
        $selstate = (!empty($_REQUEST['selstate']))? $_REQUEST['selstate'] : "";
        $selcity = (!empty($_REQUEST['selcity']))? $_REQUEST['selcity'] : "";
        $password =(!empty($_REQUEST['password']))? $_REQUEST['password'] : "";
        $zone =(!empty($_REQUEST['zone']))? $_REQUEST['zone'] : "";
        $userid_for_registration = (!empty($_REQUEST['userid_for_registration']))? $_REQUEST['userid_for_registration'] : "";
        $referralcode = (!empty($_REQUEST['referralcode']))? $_REQUEST['referralcode'] : ""; 
		...
		/*check phone number exists*/
		$additional_data = array('first_name' => $fname,'last_name' => $lname,'phone' => $phone,'Address' => $user_address,'City' => $selcity,'State_Code' => $selstate,'Zip' => $postalcode,'Zone_ID' => $zone,'alternate_phone' => $phone,'gender' => $gender);
        $id = $this->ionAuth->register($username, $password, $uemail, $additional_data, $accountGroups,$userid_for_registration);
        $lonlngArr = $this->CommonController->getLocationLatLong($postalcode);
        if ($lonlngArr) {
        	 $lat = $lonlngArr['lat'];
        	 $lng = $lonlngArr['lng'];
        }else{
    	...
    	if(!$id){            
        	...
        	$message1 = '<div class="succ_messs_form">
            <span style="text-align:center; margin-left: 0px;">Thank you for registering your businesses sponsor with SavingsSites.</span><br /> 
            <span style="text-align:center; margin-left: 0px;">A confirmation email has been sent to your email address.</span> <br />
            <span style="text-align:center; margin-left: 0px;">BE SURE to check your JUNK email folder if the email does</span> <br />
            <span style="text-align:center; margin-left: 0px;">not appear in your inbox. The confirmation email will have</span><br />
            <span style="text-align:center; margin-left: 0px;">a link to your SavingsSites dashboard where you can confirm </span><br />
        	...
        	$message_body1="<div style='border:1px solid #900; padding:5px;'><font size='4'>Dear ".$username.",<br /><br />Thank you for registering with SavingsSites.<br/><br/>Please click the link below:<br /><br />".$link."<br/><br/>
            Clicking the link above will confirm your registration and you will be able to login to your new business account and start creating  your first ad. As soon as the Directory Manager approves your account your ad will become visible within the SavingsSites directory  in which you registered. The link above will take you to the main SavingsSites page so that you can login to your business dashboard.<br /><br />
            Best Regards,<br />
            Savings Sites<br/>
            <a href='savingssites.com'>savingssites.com</a>
        	...
        	$message_body= '<body><div style="color:#000;background: #fff;width: 70%;margin: 50px auto;padding: 11px;border: 1px solid #ccc;font-size: 14px;line-height: 26px;box-shadow: 0px 1px 11px rgb(111 110 122 / 52%);"><div style="text-align: center;background: #f2f2f2;padding: 17px;margin-bottom: 20px"><img style="width: 240px;" src="https://cdn.savingssites.com/logo-green.png"/></div>
                ...
                <div style="background-color:#FFF; margin-top:10px; margin-bottom:10px; min-height:300px; padding:15px;">
                    ...
                    <div style="background-color:#fff; height:60px;"></div></body>';
            $template_subject="Savings Sites Sponsor Account Registration";
        	$this->CommonController->SendMail('',$uemail,$template_subject,$message_body);
        }
        echo json_encode($message);
        die;
...
//var_dump($update_zone_menu); echo 4; exit;
	...
	public function check_login_type_exists($username,$login_type,$zoneid=0){
 		$emailParameter = "a.username='".$username."'";
		$sql="SELECT a.id FROM users a, users_groups b where a.id=b.user_id and b.group_id=".$login_type." and ".$emailParameter;
	...
	public function check_login_type(){ 
		...
		$zoneid =(!empty($_REQUEST['zoneid']))? $_REQUEST['zoneid'] : "";
 		$msgArr = ['4'=> 'Zone Login', '5' => 'Business Login','8' => 'Organization Login','10' => 'Neighbors Login. Want to login as Visitor?','7' => 'Visitor Login','17' => 'Sponsor Login'];
		$msg = isset($msgArr[$login_type])?$msgArr[$login_type]:'';
		$check_login = $this->check_login_type_exists($username,$login_type);
		if($check_login == 1){
			$response=1;
		}else if($check_login == 0){
			$response=['response'=>'You are not a valid user for '.$msg];
		} 			 
	...
	**/
	...
	function login_from_zone_page($username = '',$password = '',$zoneid = '', $via = ''){
		...
		if($this->session->get('user_id') !=''){  
			...
			$subdomainZone = $this->CommonController->SelectDataMultiWay('sales_zone','subdomainZone','column',['id' => $zoneid]);
			redirect()->to('https://'.$subdomainZone.'.savingssites.com/Zonedashboard/zoneinformation');
		}
		
		// gen url link
		$urlsql = "SELECT * FROM sales_zone WHERE id = '".$userzone."'";
        $urlArr = $this->CommonController->SelectRawquery($urlsql,'row');
		$subdomainZoneprefix = $urlArr->subdomainZone;
		$host = $_SERVER['HTTP_HOST'];
        $explodehost = explode('.', $host);
		if(in_array('loc', $explodehost)){
            $zoneurl = 'https://'.$subdomainZoneprefix.'.savingssites.loc';
        }else if(in_array('qa', $explodehost)){
            $zoneurl = 'https://'.$subdomainZoneprefix.'.qa.savingssites.com';
        }else{
            $zoneurl = 'https://'.$subdomainZoneprefix.'.savingssites.com';
        } 
		...
		if ($this->ionAuth->login($username, $password, $remember)){
			...
			$this->CommonController->SetCookie('subDomainUsername',$username,'','','/','');
			$subpassword = base64_encode($password);
			$this->CommonController->SetCookie('subDomainUserPass',$subpassword,'','','/','');
			$this->CommonController->SetCookie('subDomainZone',$userzone,'','','/','');
			...
			if($user_type==4){ 
				...
				$businesses = $this->Business->get_all_businesses_for_user($user_id);
				if(!empty($businesses)){	
					$settingzoneid = $this->CommonController->getsinglecolumndata('ads_setting_preferences','settingszoneid','businessid',$businesses[0]['id']);	
					if($settingzoneid == ''){
						$settingzoneid = $userzone;
					}		
					$bid = $businesses[0]['id'];
			...
			}else if($user_type==17){ 
				$auser = $this->ionAuth->user()->row(); 
				$user_id = $auser->id;
				if($login_type!='main_login'){
					$this->CommonController->setSession('session_normal_user_in_zone', array('sesuserzone'=>$userzone,'sesusertype'=>'resident_user' , 'usertype' =>$user_type), 'array');
					$this->CommonController->setSession('session_login_details', array('id'=>$user_id,'type'=>'resident_user' ,'type' =>$user_type,'zone' =>$userzone), 'array');
					$jsonArr = json_encode($_SESSION);
					$sessiondata = base64_encode($jsonArr);
					$update_zone_menu=$this->Category_management_model->update_zone_menu($userzone);
					$response='sponsor_profile!~!'.$userzone.'!~!'.$user_type.'!~!'.$sessiondata;	
				}else{
					$response='<span style="color:red">Invalid User.</span>!~!0';
				}
			}else if($user_type==8){
					...
					$session_login_details = array('type'=>$user_type,'sesuserzone'=>$zoneid,'id'=>$user_id);
					$this->CommonController->setSession('session_login_details', $session_login_details);
 					if($login_type=='main_login'){
		...
		if($via == ''){
			echo json_encode(['response'=>$response,'zone'=>$zoneurl]);
		}
	}
