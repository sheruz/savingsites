...
class Ads extends BaseController{
    ...
    public function get_certificate(){
        ...
        if($zone_id != '' && $userid != ''){
            $peekaboodeals = $this->Organization_model->getuserdealsbyid($zone_id,$userid,$deal_id ,$purchaseID,$via);
...
.bv_verfied_row .button1{ width: 100%; font-size: 30px; line-height: 36px;  }   
    ...
    if (isset($check_certficate['certificate_verify']) && $check_certficate['certificate_verify'] == 1) {
...
<div style="width: 100%;" class="container">
   <div class="row bv_cerfied_row">
      <div class="col-md-6 cerfied_left_col">
         <img src="https://cdn.savingssites.com/new-savingssites3.png" width="300";>
      </div>';
      if($via == ''){
        $html .='<div class="col-md-6 cerfied_right_col" style="text-align: right;">
   ...
   <div class="row bv_issued_row"> 
      ...
      <div class="col-md-3" style="margin-left: 0px; text-align: right;color: #DC143C;">Auction End Date:<br /> '.date('m-d-Y',strtotime($check_certficate['end_date'])).'</div></div>
      <!-- <input type="text" name=""  > -->
   ...
   if($check_certficate['username']==""){
   ...
   }elseif(isset($check_certficate['certificate_verify']) && $check_certficate['certificate_verify'] != 1 ){
           ...
           <div class="row bv_verfied_row">
               <div class="button_outer">
                <a href="'.basename($_SERVER['REQUEST_URI']).'"  data-dealid="'.$check_certficate['id'].'"  class="verifyme button1">VERIFY Deals</a>
               </div>
            </div>';
   }elseif(isset($check_certficate['certificate_verify']) && $check_certficate['certificate_verify'] == 1 ){ 
            $html .= '<div class="row bv_verfied_row '.$hidee.'">
                <div class="button_outer">
   ...
   $html .= '<div style="width: 100%;" class="container footer_cerfied_col">
      ...
      <div class="col-md-12 text-center">
         <span class="issued"><span style="color:#DC143C;text-decoration: none ">*</span>1-time use. Never expires. Issued at: '.date('m-d-Y', strtotime($check_certficate['purchasedAt'])).'</span>
      </div>
...
// -- load peekaboo auctions on advertisement load
    ...
    public function temp_ads(){
        ...
        $user_id=!empty($_REQUEST['user_id']) ? $_REQUEST['user_id'] : 0;
        $charval=!empty($_REQUEST['charval']) ? $_REQUEST['charval'] : '';
        $sortbyvalue=!empty($_REQUEST['sortbyvalue']) ? $_REQUEST['sortbyvalue'] : '';
        $albhabetvalue=!empty($_REQUEST['albhabetvalue']) ? $_REQUEST['albhabetvalue'] : '';
        $deal_title_ad_id=!empty($_REQUEST['deal_title_ad_id']) ? $_REQUEST['deal_title_ad_id'] : 0;
        $deal_title_type=!empty($_REQUEST['deal_title_type']) ? $_REQUEST['deal_title_type'] : '';
        $deal_title=!empty($_REQUEST['deal_title']) ? $_REQUEST['deal_title'] : '';
        $food_deliver=!empty($_REQUEST['food_deliver']) ? $_REQUEST['food_deliver'] : 0;
        ...
        if($from_where=='home_page_ads'){
            ...
            $data['adlist'] = $this->Ads_model->get_ads_for_all_athena_latest_working_anishnew($zone_id,$lowerlimit,$upperlimit,'',$cat_id,$subcat_id,0,0,0,$charval,0,0,0,0,0,0,0,0,'show_all_offers','',$sortbyvalue);
            $data['total_post'] = $this->Ads_model->get_ads_for_all_athena_latest_working_anishtotal($zone_id,$lowerlimit= 0 ,$upperlimit = 10000000,$cat_id,$subcat_id,0,0,0,0,$charval,0,0,0,0,0,0,0,0,'show_all_offers','',$sortbyvalue);
        }else if($from_where=='show_temp_ads'){ // business coming soon
            if($subcat_id==0){  // 0 means business comming soon
                $subCatId=-99;
            }
            ...
            $data['call_again'] = !empty($_REQUEST['call_again']) ? $_REQUEST['call_again'] : false;
            $data['adlist'] = $this->Ads_model->get_ads_for_all_athena_latest_working_anishnew($zone_id,$lowerlimit,$upperlimit,'',$cat_id,$subcat_id,0,0,0,'',0,'','',$food_deliver,0,0,0,0,'show_ads_specific_sub_category');
            $data['total_post'] =  $this->Ads_model->get_ads_for_all_athena_latest_working_anishtotal($zone_id,$lowerlimit= 0  ,$upperlimit = 10000000,'',$cat_id,$subcat_id);
        }else if($from_where=='show_ads_specific_category'){ // show_ads_specific_category
            ...
            $data['adlist'] = $this->Ads_model->get_ads_for_all_athena_latest_working_anishnew($zone_id,$lowerlimit,$upperlimit,$search_value='',$cat_id,$subcat_id,$user_id=0,$buseness_id=0,$ad_id=0,$charval,$deal_title_ad_id=0,$deal_title_type='',$deal_title='',$food_deliver=0, $subcatparentid=0 , $multiplesubcatid = 0, $business_sponsor = 4,'filter-for-pboo','','','',$albhabetvalue);
            
            $data['total_post'] =  $this->Ads_model->get_ads_for_all_athena_latest_working_anishtotal($zone_id,$lowerlimit= 0  ,$upperlimit = 10000000 ,$search_value='',$cat_id,$subcat_id,$user_id=0,$buseness_id=0,$ad_id=0,$charval,$deal_title_ad_id=0,$deal_title_type='',$deal_title='',$food_deliver=0, $subcatparentid=0 , $multiplesubcatid = 0, $business_sponsor = 4,'filter-for-pboo','','','',$albhabetvalue);
        }elseif($from_where =='sponsor_businesses_menu_online_delievry_order'){ 
            $data['adlist'] = $this->ad->get_ads_for_all_athena_latest_working_anish($zone_id,$lowerlimit,$upperlimit,$search_value='',$cat_id=0,$sub_cat_id=0,$user_id=0,$buseness_id=0,$ad_id=0,$charval,$deal_title_ad_id=0,$deal_title_type='',$deal_title='',$food_deliver=0, $subcatparentid=0 , $multiplesubcatid = 0, $business_sponsor = 4);
            $total_post = $this->ad->get_ads_for_all_athena_latest_working_anish($zone_id,$lowerlimit= 0  ,$upperlimit = 10000000,$search_value='',$cat_id=0,$sub_cat_id=0,$user_id=0,$buseness_id=0,$ad_id=0,$charval,$deal_title_ad_id=0,$deal_title_type='',$deal_title='',$food_deliver=0, $subcatparentid=0 , $multiplesubcatid = 0, $business_sponsor = 4);
        }elseif($from_where =='show_data_miles'){ 
            $data['adlist'] = $this->Ads_model->get_ads_for_all_athena_latest_working_anishnew($zone_id,$lowerlimit,$upperlimit,$search_value,$cat_id,$subcat_id,$user_id,0,0,'',0,'','',$food_deliver,0,0,0,0,'show_data_miles','',$sortbyvalue,$albhabetvalue,$lat,$lng);
            $data['total_post'] = $this->Ads_model->get_ads_for_all_athena_latest_working_anishnew($zone_id,$lowerlimit= 0  ,$upperlimit = 10000000,'',$cat_id,$subcat_id,$user_id,0,0,'',0,'','',$food_deliver,0,0,0,0,'show_data_miles','total',$sortbyvalue,$albhabetvalue,$lat,$lng);
        } 
        ...
        if(!empty($data['adlist'])){
            ...
            foreach($data['adlist'] as $key=>$addetails){
                if($from_where=='filter-for-pboo'){ 
                    ...
                    $data['adlist'][$key]['peekaboolist'] = $this->Ads_model->show_dealsnew($addetails['adid'], $zone_id,'',$addetails['username'], $addetails['busid'],$addetails['tbl_user_id'],$addetails['bs_name'],$addetails['aucid'],$addetails['insert_via_csv']) ;
        ...
        foreach($data['adlist'] as $key=>$adddetails){
            ...
            if($adddetails['bs_id'] >0 && $userId > 0){
...
// Res User Account Business Newsletter (TAB)
    ...
    public function profile_update($id=0,$fn='',$ln='',$phone='',$carrier=''){
        $id = isset($_REQUEST['id'])?trim($_REQUEST['id']):$id;
        $fn = isset($_REQUEST['fn'])?trim($_REQUEST['fn']):$fn;
        $ln = isset($_REQUEST['ln'])?trim($_REQUEST['ln']):$ln;
        $res_gen = isset($_REQUEST['res_gen'])?trim($_REQUEST['res_gen']):'';
        $phone = isset($_REQUEST['phone'])? str_replace("-", "", $_REQUEST['phone']):$phone;
        $carrier = isset($_REQUEST['carrier'])?trim($_REQUEST['carrier']):$carrier;
        $alternatephone = isset($_REQUEST['alternatephone'])?$_REQUEST['alternatephone']:[];
        $email = trim($_REQUEST['res_email']);
        $res_address = isset($_REQUEST['res_address'])?trim($_REQUEST['res_address']):'';
        $res_state = isset($_REQUEST['res_state'])?trim($_REQUEST['res_state']):'';
        $res_city = isset($_REQUEST['res_city'])?trim($_REQUEST['res_city']):'';
        $res_zip = isset($_REQUEST['res_zip'])?trim($_REQUEST['res_zip']):'';

        if ($id == '' || $fn == '' || $ln == '' || $phone == '' || $email == '' ) {
            echo json_encode(array('type' => 'danger', 'msg'=> 'All Fields are required'));
            die;
        }
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            echo json_encode(array('type' => 'danger', 'msg'=> "$email is a valid email address"));
            die;
        }

        $this->Users->get_profile_update($id,$fn,$ln,$phone,$carrier,$email,$alternatephone,$res_gen,$res_address,$res_state,$res_city,$res_zip); 
    }
  ...
  /*
    ...
    */
    ...
    public function fetchMyAccountData(){
        ...
        $catval = isset($_REQUEST['catval'])?$_REQUEST['catval']:[];
        $subcatArr1 = isset($_REQUEST['subcatArr'])?$_REQUEST['subcatArr']:[];
        $residentshareurl = isset($_REQUEST['residentshareurl'])?$_REQUEST['residentshareurl']:'';
        if($catval == 0){
            $catval = [];
        }
        $child_type = isset($_REQUEST['cat_type'])?$_REQUEST['cat_type']:''; 
        $day = isset($_REQUEST['day'])?$_REQUEST['day']:[]; 
        $qrlink = base_url().'/qrscan?userid='.$user_id.'&zoneid='.$zone_id.'';
        if($type == 'refergenlinkQR'){
            $this->QRController->add_data($qrlink,$user_id);
        }
        $peekaboodeals = $this->Organization_model->getuserdeals($zone_id,$user_id,$type);
        $referdata = $this->get_refer_link($user_id);
        $code = $referdata;
        

        $category_list_food = $this->Category_new_model->get_products_details($zone_id,1,'catads'); 
        $category_list = $this->Category_new_model->get_products_details($zone_id,0,'catads');    
        $combinefood = array_merge($category_list_food,$category_list);
        $datacatArr = [];
        foreach ($combinefood as $ck => $cv) {
            $datacatArr[$cv['catname']] = $cv['catid'];  
        }

        /*get fav business list data*/
        $userofferquery = "SELECT * FROM user_offer_status where user_id ='".$user_id."'";
        $userofferres = $this->CommonController->SelectRawquery($userofferquery);
        $QR_Image = $this->CommonController->getsinglecolumndata('users','qrimage','id',$user_id);
        if($QR_Image == ''){
            $this->QRController->add_data($qrlink,$user_id); 
            $QR_Image = $this->CommonController->getsinglecolumndata('users','qrimage','id',$user_id);  
        }
        ...
        if(count($catval) > 0){
            $subcatArr = [];
            foreach ($catval as $k1 => $v1) {
                $category_list_food = $this->Category_new_model->get_products_details($zone_id,$v1,'ads');
                if(count($category_list_food) > 0){
                    foreach ($category_list_food as $k2 => $v2) {
                        $subcatArr[] = $v2;    
                    }
                }
            }
        ...
        $deallink = $residentshareurl.'/'.$code;
        $html = '';
        $count = 0;
        $sr = 0;
        $groupArr1 = $this->CommonController->getbusinessloginbutton($user_id);
        ...
        if($type == 'userupdate'){
            ...
            $gender = str_replace("%20"," ",$user['gender']);
            $email = str_replace("%20"," ",$user['email']);
            $address1 = str_replace("%20"," ",$user['address1']);
            $city_name = str_replace("%20"," ",$user['city_name']);
            $state_name = str_replace("%20"," ",$user['state_name']);
            $post_code = str_replace("%20"," ",$user['post_code']);
            $count = count($user['alternateno']);
            if($gender == 1){$selected1 = 'selected';}
            if($gender == 2){$selected2 = 'selected';}
            if($gender == 3){$selected3 = 'selected';}
            $html .= '<input type="hidden" name="zoneid" id="zoneid" value="'.$zone_id.'"/>
                    <div class="row gggg resident_dash_bg">
                        ...
                        foreach($user['alternateno'] as $v){ 
                            ...
                            $html .='<div class="form-group aComment">  
                                        ...
                                        <button style="width: 5%;float: left;height:50px;" type="button" class="btn btn-danger rcellpalt">X</button>
                                    </div>';
                        }
                            $html .= '<div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <span><label class="m_top_8i">Gender:</label>
                                        <select class="form-control" id="res_gen">
                                            <option value="1" '.$selected1.'>Male</option>
                                            <option value="2" '.$selected2.'>Female</option>
                                            <option value="3" '.$selected3.'>Others</option>
                                        </select>
                                        </span>
                                    </div>
                                    <div class="form-group">
                                        <span><label class="m_top_8i">Email Address:</label>
                                            <input type="text" id="res_email" class="form-control" value="'.$email.'"/>
                                        </span>
                                    </div>
                                    <div class="form-group">
                                        <span><label class="m_top_8i">Address:</label>
                                            <textarea id="res_address" class="form-control">'.$address1.'</textarea>
                                        </span>
                                    </div>
                                    <div class="form-group">
                                        <span><label class="m_top_8i">State:</label>
                                            <input type="text" id="res_state" class="form-control" value="'.$state_name.'"/>
                                        </span>
                                    </div>
                                    <div class="form-group">
                                        <span><label class="m_top_8i">City:</label>
                                            <input type="text" id="res_city" class="form-control" value="'.$city_name.'"/>
                                        </span>
                                    </div>
                                    <div class="form-group">
                                        <span><label class="m_top_8i">Zip:</label>
                                            <input type="text" id="res_zip" class="form-control" value="'.$post_code.'"/>
                                        </span>
                                    </div>
                                    <div class="form-group">  
            ...
            <div class="row gggg resident_dash_bg">
                <div class="col-md-12 offset-md-30">
                    <div class="update_profile_form cus-bg-update">
                        <div class="update_profile_form_inner"> 
                            <span id="success" style="font-weight:bold; color:#090;display:block; text-align:center; display:none;"></span>
                                <h2>All Send Deals</h2>
                                <p>The deals send by you are listed below:</p>            
                                <table class="table table-striped">
                                    ...
                                    if(count($peekaboodeals) > 0){
                                        ...
                                        <td class="verifylink"> <a href="#" class="clickview" data-productid="'.$peekaboodeals["deal_id"].'" data-porchaseid="'.$peekaboodeals["purchaseid"].'" dealtype="send">Click to view</a></td>
                                        <td>'.$peekaboodeals['certreciever'].'</td>
                                        </tr>';
                                        }
                                    }
                            ...
                            <span id="success" style="font-weight:bold; color:#090;display:block; text-align:center; display:none;"></span>
                                ...
                                <table class="table table-striped">
                                    ...
                                    if(count($peekaboodeals) > 0){
                                        foreach($peekaboodeals as $peekaboodeals){
                                            $certtype = 'Purchased';
                                            if($peekaboodeals['giftsenderemail'] != ''){
                                                $certtype = 'Gifted';
                                            }
                                        $html .= '<tr id="certificate'.$peekaboodeals['id'].'">
                                        <td>'.$peekaboodeals['page_title'].'</td>
                                            ...
                                            <div class="bv_pbg_col"><input type="text"  id="referlinktext" style="width:300px;" readonly value="'.$link.'"/> 
                                                <div class="bv_referal_btn">
                                                    <button class="btn btn-info" type="button" id="copytoclip">copy</button>
                                                        ...
                                                        <a class="btn btn-info" id="twitter" href="http://www.twitter.com/share?url='.$link.'" target="_blank">Share on X</a> 
                                                        <button>Edit</button>
                                                </div>
                                            </div>
                                        </div>
                ...
                <div class="col-md-12 offset-md-30">
                    <div class="update_profile_form cus-bg-update">
                        <div class="update_profile_form_inner"> 
                            <div class="main_content_outer"> 
                                <div class="content_container">
                                    <div class="refer_code_note">$8 Cash Certificate Account Balance<br>for Every New Deal Purchase!, Both you the Referrer and your referred User Purchase Deal<br>We will even apply the free ones towards the most expensive cash certs!</div>
                                    <div class="refer_code_note">Your refer Code is:- <strong><span class="refertext">'.$code.'</span></strong></div>
                                    <div class="refer_invite_col">
                                        <div class="bv_referal_left">
                                            <img src="https://i.postimg.cc/tgy1Xb9q/tied-working.png"/>
                                        ...
                                        <div class="bv_referal_right">
                                            <div class="container_tab_header">Share Deal by Email</div>
                                            <div class="bv_pbg_col"><input type="email"  id="referlinkmail" placeholder="Enter Email Address"/> <button class="btn btn-info" type="button" id="mailsend">Send Mail</button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="bv_share_link">
                                        ...
                                        <div class="bv_sharelink_right">
                                            <div class="container_tab_header">Share Your Invite Link</div>
                                            <div class="bv_pbg_col"><input type="text"  id="referlinktext1" style="width:300px;" readonly value="'.$code.'"/>
                                                <input type="hidden"  id="referlinktext" style="width:300px;" readonly value="'.$deallink.'"/> 
                                                <div class="bv_referal_btn">
                                                    <button class="btn btn-info" type="button" id="copytoclip">copy</button>
                                                        <a class="btn btn-info" id="fb" href="https://www.facebook.com/sharer.php?u='.$deallink.'" target="_blank">Share on facebook</a> 
                                                        <a class="btn btn-info" id="twitter" href="http://www.twitter.com/share?url='.$deallink.'" target="_blank">Share on X</a> 
                                                        <a class="btn btn-info editresidentinviteurl" href="javascript:void(0)" onclick="editsharelinkresidentenable();" >Edit</a>
                                                        <a class="btn btn-info updateresidentinviteurl green hide" href="javascript:void(0)" onclick="updatesharelinkresidentenable();" >Update</a> 
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="col-md-12 offset-md-30">
                    <div class="update_profile_form cus-bg-update">
                        <div class="update_profile_form_inner"> 
                            <div class="main_content_outer"> 
                                <div class="content_container">
                                    <div class="refer_code_note">5 Free Discounted Cash Certificates<br>for Every Business Approve Your Refer!<br>We will even apply the free ones towards the most expensive cash certs!</div>
                                    <div class="refer_code_note">Your QR Code is Below:- <strong><span class="refertext"></span></strong></div>
                                    <div class="refer_invite_col">
                                        <div class="bv_referal_left ss2">

                                        <div class="qrcode" id="qrcode" style="width: 80%;margin:auto;padding: 50px;background: #fff;">
                                            <img src="https://cdn.savingssites.com/'.$QR_Image.'"/>
                                        </div>
                                        <p><button id="printQR" onclick="printDiv(qrcode);" class="btn btn-info">Print QR Code</button></p>
                                        </div>
                                        <div class="bv_referal_right">
                                            <div class="container_tab_header">Share QR by Email</div>
                                            <div class="bv_pbg_col"><input type="email"  id="qremail" placeholder="Enter Email Address"/> <button class="btn btn-info" type="button" id="generateQRemail">Send Mail</button>
                                            </div>
                                        </div>
                                    </div>
                                    </div>
        ...
        }else if($type == 'subscribebusiness'){
            $categoryArr = '';
            $ullistsubcat = '';
            foreach ($datacatArr as $ck => $cv) {
                if($catval == $cv){
                    $catselect = 'selected';
                }else{
                    $catselect = '';
                }
                $categoryArr .=  '<option '.$catselect.' value="'.$cv.'">'.$ck.'</option>';
            }
            if(count($catval) > 0){
                foreach ($subcatArr as $sk => $sv) {
                    if(in_array($sv['id'], $subcatArr1)){
                        $checksubcat = 'checked';
                    }else{
                        $checksubcat = '';
                    }
                    $ullistsubcat .=  '<li><input '.$checksubcat.' class="subcatcheck" subcatidcheck="'.$sv['id'].'" type="checkbox"/> <span>'.$sv['name'].'</span></li>';
                }
            }
            $mon = $tue = $wed = $thu = $fri = $sat = $sun = $all = $noall = ''; 
            if(in_array(1,$day)){$mon = 'checked';}
            if(in_array(2,$day)){$tue = 'checked';} 
            if(in_array(3,$day)){$wed = 'checked';} 
            if(in_array(4,$day)){$thu = 'checked';} 
            if(in_array(5,$day)){$fri = 'checked';} 
            if(in_array(6,$day)){$sat = 'checked';} 
            if(in_array(7,$day)){$sun = 'checked';} 
            if(in_array(8,$day)){$all = $mon = $tue = $wed = $thu = $fri = $sat = $sun = 'checked';} 
            if(in_array(9,$day)){$noall = 'checked';}
            $html .= '<input type="hidden" name="zoneid" id="zoneid" value="'.$zone_id.'"/>
            <div class="row gggg resident_dash_bg">
                <div class="col-md-12 offset-md-30">
                    <div class="update_profile_form cus-bg-update">
                            ...
                            <div class="main_content_outer"> 
                                <div class="content_container">
                                    <div class="refer_invite_col">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <ul id="buisnessday" class="form-control">
                                                    <li style="float:left;width:8%;"><input dayid="8" type="checkbox" '.$all.' class="daycheckall"/> All Day</li>
                                                    <li style="float:left;width:9%;"><input dayid="1" type="checkbox" '.$mon.' class="daycheck" /> Monday</li>
                                                    <li style="float:left;width:9%;"><input dayid="2" type="checkbox" '.$tue.' class="daycheck"/> Tuesday</li>
                                                    <li style="float:left;width:11%;"><input dayid="3" type="checkbox" '.$wed.' class="daycheck"/> Wednesday</li>
                                                    <li style="float:left;width:10%;"><input dayid="4" type="checkbox" '.$thu.' class="daycheck"/> Thursday</li>
                                                    <li style="float:left;width:8%;"><input dayid="5" type="checkbox" '.$fri.' class="daycheck" /> Friday</li>
                                                    <li style="float:left;width:10%;"><input dayid="6" type="checkbox" '.$sat.' class="daycheck" /> Saturday</li>
                                                    <li style="float:left;width:8%;"><input dayid="7" type="checkbox" '.$sun.' class="daycheck" /> Sunday</li>
                                                    <li style="float:left;width:24%;"><input dayid="9" type="checkbox" '.$noall.' class="daychecknot" /> No I don’t want email notices.</li>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-10">
                                                <select id="buisnesssnap" multiple class="form-control">
                                                    '.$categoryArr.'    
                                                </select>
                                            </div>
                                        </div>
                                        <br>
                                        <div class="row">
                                            <div class="col-md-12" id="subcatdiv">
                                                <div style="height:200px;overflow-y:auto;width:95%;margin:auto">
                                                    <ul>'.$ullistsubcat.'</ul>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                               <button class="btn btn-info savesubcatdeals">Submit</button>
                                            </div>
                                        </div>
                                    </div>
                                <div>
        ...
        }else if($type == 'showsubscribebusiness'){
          $html .= '<input type="hidden" name="zoneid" id="zoneid" value="'.$zone_id.'"/>
            <div class="row gggg resident_dash_bg">
                <div class="col-md-12 offset-md-30">
                    <div class="update_profile_form cus-bg-update">
                        <div class="update_profile_form_inner"> 
                            <span id="success" style="font-weight:bold; color:#090;display:block; text-align:center; display:none;"></span>
                                <h2>SNAP Business List</h2>  
                                <p>Businesses create offers for a limited time and notify users via email or text. This page displays a list of businesses that have generated time-limited offers.</p>          
                                <table class="table table-striped">
                                    <thead>
                                        <tr>
                                            <th>Business Name</th>
                                    ...
                                    <tbody>';
                                    if(count($userofferres) > 0){
                                        foreach($userofferres as $pk => $pv){
                                            $busqry = "SELECT name FROM business where id ='".$pv['createdby_id']."'";
                                            $busres = $this->CommonController->SelectRawquery($busqry,'row');
                                            $snapStatus = 'IN-Active';
                                            $sendtype = '';
                                            $snaptimeshow = '';
                                            if($pv['status'] == 1){
                                                $snapStatus = 'Active';
                                            }
                                            if($pv['send_type'] == 1){$sendtype = 'Email';
                                            }else if($pv['send_type'] == 2){$sendtype = 'Text';
                                            }else if($pv['send_type'] == 3){$sendtype = 'Email & Text';}
                                            if($pv['snap_time_id'] == 0){$snaptime = '';}
                                            if($pv['snap_days_id'] != ''){
                                                $snapdayArr = explode(',', $pv['snap_days_id']);
                                                foreach ($snapdayArr as $dv) {
                                                    if($dv == 1){$snaptimeshow .= '<span>Sunday</span><br>';}
                                                    if($dv == 2){$snaptimeshow .= '<span>Monday</span><br>';}
                                                    if($dv == 3){$snaptimeshow .= '<span>Tuesday</span><br>';}
                                                    if($dv == 4){$snaptimeshow .= '<span>Wednesday</span><br>';}
                                                    if($dv == 5){$snaptimeshow .= '<span>Thursday</span><br>';}
                                                    if($dv == 6){$snaptimeshow .= '<span>Friday</span><br>';}
                                                    if($dv == 7){$snaptimeshow .= '<span>Saturday</span><br>';}
                                                }
                                            }
                                        $html .= '<tr id="'.$pv['id'].'">
                                        <td>'.$busres->name.'</td>
                                        <td>'.$snaptimeshow.'</td>
                                        <td>'.$snaptime.'</td>
                                        <td class="storedstatus">'.$snapStatus.'</td>
                                        <td>'.$sendtype.'</td>
                                        <td><button class="btn btn-danger snapchangestatus" types="inactive" ids="'.$pv['id'].'">In-Active</button> <button  ids="'.$pv['id'].'" class="btn btn-success  snapchangestatus"  types="active">Active</button></td>
                                        </tr>';
                                        }
                                    }
                            $html .= '</tbody></table></div></div></div></div>'; 
        }else if($type == 'manyusersinglelogin'){
            $html .= '<input type="hidden" name="zoneid" id="zoneid" value="'.$zone_id.'"/>
            <div class="row gggg resident_dash_bg">
                <div class="col-md-12 offset-md-30">
                    <div class="update_profile_form cus-bg-update">
                        <div class="update_profile_form_inner"> 
                        <h2>Multiple Login Single User</h2>  
                                        ...
                                        <div class="row">
                                            ...
                                            if(count($groupArr) > 0){
                                                foreach ($groupArr as $gv) {
                                                    if($gv == 5){
                                                        $html .= '<li class="loginbutton"><button type="button" phone="'.$phonelogin.'" email="'.$emaillogin.'" group="5" class="btn btn-info singlelogin">Login as Business</button></li>';
                                                    }
                                                    if($gv == 10){
                                                        $html .= '<li class="loginbutton"><button type="button"  phone="'.$phonelogin.'" email="'.$emaillogin.'" group="10" class="btn btn-info singlelogin">Login as Resident</button></li>';
                                                    }
                                                    if($gv == 8){
                                                        $html .= '<li class="loginbutton"><button type="button" phone="'.$phonelogin.'" email="'.$emaillogin.'" group="8" class="btn btn-info singlelogin">Login as Organization</button></li>';
                                                    }
                                                    if($gv == 15){
                                                        $html .= '<li class="loginbutton"><button type="button"  phone="'.$phonelogin.'" email="'.$emaillogin.'" group="15" class="btn btn-info singlelogin">Login as Employee</button></li>';
                                                    }
                                                    if($gv == 7){
                                                        $html .= '<li class="loginbutton"><button type="button"  phone="'.$phonelogin.'" email="'.$emaillogin.'" group="7" class="btn btn-info singlelogin">Login as Visitor</button></li>';
                                                    }
                                                }
                                              }else{
        ...
        }else if($type == 'addsnaptime'){
            $snaptimeqry = "SELECT * FROM snap_start_time Order by id";
            $snaptimeArr = $this->CommonController->SelectRawquery($snaptimeqry);

            $snapdiscountqry = "SELECT * FROM snap_discount_table Order by id";
            $snapdiscountArr = $this->CommonController->SelectRawquery($snapdiscountqry);
            
            $html .= '<div class="container margin50 cus-bg-update"><div class="row">
                <div class="col-md-12 text-center"><h2 class="SnpSettings">SNAP Filters <a href="#"><u>read more</u></a></h2></div>
            </div>
            <br>
            <div class="row">
                <div class="col-md-8">
                    <div class="row">
                        ...
                        <div class="col-md-7"><h4 class="boldhead">Available Time Period</h4></div>
                        <div class="col-md-2"></div>
                    </div>
                </div>
                <div class="col-md-4">
                <h4 class="text-center boldhead">Minimum Discount Required</h4></div>
            </div>
            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="row">
                        <div class="col-md-1 timehead">
                        <h4>Start</h4></div>
                        <div class="col-md-3">
                            <select class="form-control bussnapstarttime">';
                                foreach ($snaptimeArr as $starttime) {
                                    $html .= '<option value="'.$starttime['id'].'">'.$starttime['time'].'</option>';
                                    }
                            $html .= '</select>
                        </div>
                         <div class="col-md-1 timehead"><h4>End</h4></div>
                        <div class="col-md-3">
                            <select class="form-control bussnapendtime">';
                                foreach ($snaptimeArr as $endtime) {
                                    $html .= '<option value="'.$endtime['id'].'">'.$endtime['time'].'</option>';
                                }
                            $html .= '</select>
                        </div>   
                    </div>
                ...
                <div class="row">
                    <div class="col-md-3"></div>
                    <div class="col-md-7">
                        <select class="form-control" id="noofpeoplesnaptime">
                            <option value="">Select Discount</option>';
                            foreach ($snapdiscountArr as $snapdis) {
                                $html .= '<option value="'.$snapdis['id'].'">'.$snapdis['discount_percentage'].'</option>';
                            }
                            $html .= '</select>
                    </div>
                    <div class="col-md-2"></div>
            ...
            <div class="row">
                ...
                <h4>
                    <div class="weekdayscolumn"><input type="checkbox" class="perdayselect" day="1" dayword="M"/></div>
        ...
        <div class="row">
            <div class="col-md-12">
                <button class="btn btn-info w_200 cus-btn businesssnapaddtolist">Add</button>
            </div>
        </div>
        <br><br>
        ...
        <div class="row">
            <div class="col-md-12">
                <button class="btn btn-info w_200 cus-btn savesnapforbusiness hide">Update</button>
            </div>
        </div></div>';
        }else if($type == 'showsnapuser'){
            $snapuserqry = "SELECT * FROM global_snap_user_settings WHERE user_id=".$user_id."  Order by id";
            $snapuserArr = $this->CommonController->SelectRawquery($snapuserqry,'result');
            if(count($snapuserArr) > 0){
                foreach ($snapuserArr as $k => $v) {
                    ...
                    foreach ($snaptimeArr as $k1 => $v1){
                        ...
                        if($v1->dissnaparr != ''){
                            $v1->dissnapword = $this->CommonController->getsinglecolumndata('snap_discount_table','discount_percentage','id',$v1->dissnaparr);
                        }else{
                            $v1->dissnapword = '';
                        }
                        $snaptimeArr1[] = $v1;
            ...
            $html .= '<div class="container margin50 cus-bg-update"">
            <div class="row" style=" margin-bottom: 80px;">
                <div class="top-title">
                    <h2>Show SNAP Time</h2>
                    <hr class="center-diamond">
                    <p>Below are the SNAP deals offered by the user.</p>
                </div>
                <table class="display responsive-table table table-striped table-bordered" cellspacing="0" width="100%" id="managecat">
                    <thead>
                        <tr>
                            <th scope="col">Day</th>
                            <th scope="col">SNAP Time</th>
                            <th scope="col">Action</th>
                        </tr>
                    </thead>
                <tbody>';
                $mtable = $ttable = $wetable = $thtable = $ftable = $satable = $suntable = '';
               
                $mondaystarttime = isset($finalsnapbusArr[1]->starttime)?$finalsnapbusArr[1]->starttime:'';
                $mondaysnaptimeArr = isset($finalsnapbusArr[1]->snap_time)?$finalsnapbusArr[1]->snap_time:[];
               
                $tuesdaystarttime = isset($finalsnapbusArr[2]->starttime)?$finalsnapbusArr[2]->starttime:'';
                $tuesdaysnaptimeArr = isset($finalsnapbusArr[2]->snap_time)?$finalsnapbusArr[2]->snap_time:[];
               
                $wednesdaystarttime = isset($finalsnapbusArr[3]->starttime)?$finalsnapbusArr[3]->starttime:'';
                $wednesdaysnaptimeArr = isset($finalsnapbusArr[3]->snap_time)?$finalsnapbusArr[3]->snap_time:[];
               
                $thursdaystarttime = isset($finalsnapbusArr[4]->starttime)?$finalsnapbusArr[4]->starttime:'';
                $thursdaysnaptimeArr = isset($finalsnapbusArr[4]->snap_time)?$finalsnapbusArr[4]->snap_time:[];
               
                $fridaystarttime = isset($finalsnapbusArr[5]->starttime)?$finalsnapbusArr[5]->starttime:'';
                $fridaysnaptimeArr = isset($finalsnapbusArr[5]->snap_time)?$finalsnapbusArr[5]->snap_time:[];
               
                $saturdaystarttime = isset($finalsnapbusArr[6]->starttime)?$finalsnapbusArr[6]->starttime:'';
                $saturdaysnaptimeArr = isset($finalsnapbusArr[6]->snap_time)?$finalsnapbusArr[6]->snap_time:[];
               
                $sundaystarttime = isset($finalsnapbusArr[7]->starttime)?$finalsnapbusArr[7]->starttime:'';
                $sundaysnaptimeArr = isset($finalsnapbusArr[7]->snap_time)?$finalsnapbusArr[7]->snap_time:[];
                $commontabe = '';
               for ($i=1; $i <=7 ; $i++) { 
                  if(isset($finalsnapbusArr[$i]->snap_time) && count($finalsnapbusArr[$i]->snap_time) > 0){
                     $commontabe = '<table class="table table-striped table-bordered">';
                     $commontabe .= '<tr><th>Snap start time</th><th>Snap end time</th><th>All day</th><th>Week day</th><th>Week end</th><th>Discount %</th></tr>';
                     foreach ($finalsnapbusArr[$i]->snap_time as $k1 => $v1) {
                        ...
                        $commontabe .= '<tr><td>'.$v1->starttimeword.'</td><td>'.$v1->endtimeword.'</td><td>'.$allday.'</td><td>'.$weekdays.'</td><td>'.$weekend.'</td><td>'.$v1->dissnapword.'</td></tr>';  
                     }
                     $commontabe .= '</table>';   
                     if($i == 1){ $mtable    = $commontabe; } 
                     if($i == 2){ $ttable    = $commontabe; } 
                     if($i == 3){ $wetable   = $commontabe; } 
                     if($i == 4){ $thtable   = $commontabe; } 
                     if($i == 5){ $ftable    = $commontabe; } 
                     if($i == 6){ $satable   = $commontabe; } 
                     if($i == 7){ $suntable  = $commontabe; } 
                  }
                  
               }
                $html .= '<tr><td>Monday</td><td id="removedetailsnapdiv1">'.$mtable.'</td><td> <i user="'.$user_id.'" daycount=1 class="fa fa-trash deleteusersnaptime"></i></td></tr>
                  <tr><td>Tuesday</td><td id="removedetailsnapdiv2">'.$ttable.'</td><td> <i user="'.$user_id.'" daycount=2 class="fa fa-trash deleteusersnaptime"></i></td></tr>
                  <tr><td>Wednesday</td><td id="removedetailsnapdiv3">'.$wetable.'</td><td> <i user="'.$user_id.'" daycount=3 class="fa fa-trash deleteusersnaptime"></i></td></tr>
                  <tr><td>Thursday</td><td id="removedetailsnapdiv4">'.$thtable.'</td><td> <i user="'.$user_id.'" daycount=4 class="fa fa-trash deleteusersnaptime"></i></td></tr>
                  <tr><td>Friday</td><td id="removedetailsnapdiv5">'.$ftable.'</td><td> <i user="'.$user_id.'" daycount=5 class="fa fa-trash deleteusersnaptime"></i></td></tr>
                  <tr><td>Saturday</td><td id="removedetailsnapdiv6">'.$satable.'</td><td> <i user="'.$user_id.'" daycount=6 class="fa fa-trash deleteusersnaptime"></i></td></tr>
                  <tr><td>Sunday</td><td id="removedetailsnapdiv7">'.$suntable.'</td><td> <i user="'.$user_id.'" daycount=7 class="fa fa-trash deleteusersnaptime"></i></td></tr>
                </tbody>
                </table>
                </div>
            </div>'; 
        }else if($type == 'claimedsnapdeal'){
            $sql = "SELECT * FROM global_snap_user_claim_time where user_id='".$user_id."'";
            $res = $this->CommonController->SelectRawquery($sql,'result');
            $html .= '<div class="container margin50 cus-bg-update"">
            <div class="row" style=" margin-bottom: 80px;">
                <div class="top-title">
                    <h2>Claimed Snap Deals</h2>
                    <hr class="center-diamond">
                    <p>Below are the SNAP deals claimed by the user.</p>
                </div>
                <table class="display responsive-table table table-striped table-bordered" cellspacing="0" width="100%" id="managecat">
                    <thead>
                        <tr>
                            <th scope="col">Day</th>
                            <th scope="col">SNAP In Time</th>
                            <th scope="col">SNAP Out Time</th>
                        </tr>
                    </thead>
                <tbody>';
                foreach ($res as $k1 => $v1) {
                    $starttimeword = $this->CommonController->getsinglecolumndata('snap_start_time','time','id',$v1->snaptimeIn); 
                        $endtimeword = $this->CommonController->getsinglecolumndata('snap_start_time','time','id',$v1->snaptimeout);
                        $date = $t = date('Y-m-d H:i:s',strtotime($v1->created_at));
                    $html .= '<tr><td>'.$date.'</td><td>'.$starttimeword.'</td><td>'.$endtimeword.'</td></tr>';        
                }
    ...
    public function fetchMyAccountDataedit(){
        $html = $categoryArr = $ullistsubcat = '';
        $count = 0; $subcatArr = [];
        ...
        $sql = "SELECT * FROM users_favorites_deals where userId='".$user_id."'";
        $res = $this->CommonController->SelectRawquery($sql,'row');
        $day = isset($res->day)?$res->day:'';
        $catId = isset($res->catId)?$res->catId:'';
        $subcatId = isset($res->subcatId)?json_decode($res->subcatId):'';
        if($day != ''){
            $dayArr = json_decode($day);
            if(isset($dayArr[0]) && $dayArr[0] == 9){
                $html .= '<input type="hidden" name="zoneid" id="zoneid" value="'.$zone_id.'"/>
                    <div class="row gggg resident_dash_bg">
                    <div class="col-md-12 offset-md-30">
                    <div class="update_profile_form cus-bg-update">
                            ...
                            <div class="main_content_outer"> 
                                <div class="content_container">
                                    <div class="refer_invite_col">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <h2>No Fav SubCat Business Found</h2>   
                                            </div>
                                        </div>
                                    </div>
                                <div>
                ...
                echo json_encode(array('html' => $html,'count' => 0,'catval'=>[]));
                die;
            }
            if($catId != ''){
                $catvalArr = json_decode($catId);
            }
            $category_list_food = $this->Category_new_model->get_products_details($zone_id,1,'catads'); 
            $category_list = $this->Category_new_model->get_products_details($zone_id,0,'catads');    
            $combinefood = array_merge($category_list_food,$category_list);
            ...
            if($type == 'editsubscribebusiness'){
                foreach ($datacatArr as $ck => $cv) {
                    $categoryArr .=  '<option value="'.$cv.'">'.$ck.'</option>';
                }
                if(count($catvalArr) > 0){
                    foreach ($subcatArr as $sk => $sv) {
                        if(in_array($sv['id'], $subcatId)){
                            $checksubcat = 'checked';
                        }else{
                            $checksubcat = '';
                        }
                ...
                $mon = $tue = $wed = $thu = $fri = $sat = $sun = $all = $noall = ''; 
                if(in_array(1,$dayArr)){$mon = 'checked';}
                if(in_array(2,$dayArr)){$tue = 'checked';} 
                if(in_array(3,$dayArr)){$wed = 'checked';} 
                if(in_array(4,$dayArr)){$thu = 'checked';} 
                if(in_array(5,$dayArr)){$fri = 'checked';} 
                if(in_array(6,$dayArr)){$sat = 'checked';} 
                if(in_array(7,$dayArr)){$sun = 'checked';} 
                if(in_array(8,$dayArr)){$all = $mon = $tue = $wed = $thu = $fri = $sat = $sun = 'checked';}
                if(in_array(9,$dayArr)){$noall = 'checked';}
                $html .= '<input type="hidden" name="zoneid" id="zoneid" value="'.$zone_id.'"/>
                    <div class="row gggg resident_dash_bg">
                    <div class="col-md-12 offset-md-30">
                    <div class="update_profile_form cus-bg-update">
                            ...
                            <div class="main_content_outer"> 
                                <div class="content_container">
                                    <div class="refer_invite_col">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                               <button class="btn btn-info savesubcatdeals">Update</button>
                                            </div>
                                        </div>
                                    </div>
                                <div>
        ...
        echo json_encode(array('html' => $html,'count' => 0,'catval'=>$catvalArr));
        die;
    }

    public function get_refer_link($user_id){
        $sql = "SELECT * FROM users where id='".$user_id."'";
        $auser = $this->CommonController->SelectRawquery($sql,'row');
        if($auser->refer_code_link == ''){
            $str_result = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890abcdefghijklmnopqrstuvwxyz'; 
            $code =  time().substr(str_shuffle($str_result), 0, 10);
            
            $existscode = $this->CommonController->SelectDataMultiWay('users','refer_code_link','column',['refer_code_link' => $code]);
            if($existscode != ''){
                $this->get_refer_link($user_id);   
            }else{
            ...
            return $auser->refer_code_link;
        }
    }

    public function savesubcatbusiness(){
        $day = isset($_REQUEST['day'])?$_REQUEST['day']:[];
        $userid = isset($_REQUEST['user_id'])?$_REQUEST['user_id']:'';
        $catval = isset($_REQUEST['catval'])?$_REQUEST['catval']:[];
        $subcatArr = isset($_REQUEST['subcatArr'])?$_REQUEST['subcatArr']:[];
        $zone_id = isset($_REQUEST['zone_id'])?$_REQUEST['zone_id']:'';
        $sql = "SELECT * FROM users_favorites_deals where userId='".$userid."'";
        $res = $this->CommonController->SelectRawquery($sql,'count');
        if($res > 0){
            $this->CommonController->deleteData('users_favorites_deals',['userId'=>$userid]);
        }
        $arr = array('userId'=>$userid,'day'=>json_encode($day),'catId'=>json_encode($catval),'subcatId'=>json_encode($subcatArr),'zoneId'=>$zone_id);
        $this->CommonController->InsertData('users_favorites_deals', $arr);
        echo json_encode(['msg'=>'Sub Category Fav Saved Successfully','type'=>'success']);
        die;
    }

    public function changesnapstatus(){
        $id = isset($_REQUEST['snapid'])?$_REQUEST['snapid']:'';
        ...
        $this->CommonController->updateData('user_offer_status',$updateData,['id'=> $id]);
        echo json_encode(['msg'=>'SNAP Notification '.$msg.' Successfully','type'=>'success']);
        die;
    }

    public function stokalertmail(){
        $business_owner_id=$_REQUEST['business_owner_ids'];
        $product_ids=$_REQUEST['product_ids'];
        $sub="Deal Approval new";
        $body="Hey  , <br> Your Deal Request Approved By ".$business_owner_id." Successfully";
        $send = $this->CommonController->SendMail('',"anchalnexusfleck@gmail.com",$sub,$body);
    }

    public function updateresidentshareurl(){
        $residentshareurl = $this->myconfig->residentshareurl;
        $residentinvitecode = $_REQUEST['residentinvitecode'];
        $user_id = $_REQUEST['user_id'];

        $sql = "SELECT * FROM users where refer_code_link='".$residentinvitecode."'";
        $res = $this->CommonController->SelectRawquery($sql,'row');
        if($res == ''){
            $updateData=array("refer_code_link"=>$residentinvitecode);
            $this->CommonController->updateData('users',$updateData,['id'=> $user_id]);
            $msg = ['msg'=>'Resident share code Updated Successfully','type'=>'success','shareurl'=>$residentshareurl];
        }else{
            $msg = ['msg'=>'Share code already used, Try different code','type'=>'warning','shareurl'=>''];
        }
        echo json_encode($msg);
        die;
    }

    public function save_user_snap_filter(){
        $table              = 'global_snap_user_settings';
        ...
        if(count($snap_filter_arr) > 0){
            foreach ($snap_filter_arr as $k => $v){
                $eachday = $eachstarttime = $eachendtime = $eachnoofpeople = $eachsnapsendtype = '';
        ...
        if(count($dataArr) > 0){
            foreach ($dataArr as $dayk2 => $v2) {
                $qry = "SELECT * FROM ".$table." WHERE user_id=".$user." AND snap_week_days=".$dayk2."";
                $exists = $this->CommonController->SelectRawquery($qry,'result');
                $oldsnap_time = isset($exists[0]->snap_time)?$exists[0]->snap_time:'';

                if(count($exists) > 0){
                    $this->CommonController->deleteData($table,['user_id'=>$user,'snap_week_days'=>$dayk2]);
                }
                if($oldsnap_time != ''){
                ...
                $insertArr = array('user_id'=>$user,'snap_week_days'=>$dayk2,'snap_time'=>$snaptimeimplode,'created_for_zone'=>$zone_id,'status'=>1);
                $this->CommonController->InsertData($table, $insertArr);  
                $this->CommonController->updateData('users',['snap_settings'=>2],['id'=>$user]);   
    ...
    public function get_saved_snap_customdata(){
        $table   = 'global_snap_userbusiness_settings';
        $bus_id  = isset($_REQUEST['bus_id'])?$_REQUEST['bus_id']:'';
        $zone_id = isset($_REQUEST['zone_id'])?$_REQUEST['zone_id']:'';
        $user_id = isset($_REQUEST['user_id'])?$_REQUEST['user_id']:'';
        $htmladdsnapArr = [];
        $snap_setting = $this->CommonController->getsinglecolumndata('users','snap_settings','id',$user_id);
        $snap_setting = 1;
        
        if($snap_setting == 1){
            $qry = "SELECT * FROM ".$table." WHERE user_id=".$user_id." AND bus_id=".$bus_id."";
            $exists = $this->CommonController->SelectRawquery($qry,'result');
            $sendtype1 = '';
            if(count($exists) > 0){
                    ...
                    if(isset($v->snap_time) && count($v->snap_time) > 0){
                        foreach ($v->snap_time as $k1 => $v1) {
                            ...
                            $htmladdsnapArr[] = '<div class="snaplistsubmaindiv">
                                <div class="snaplistsubdiv header-list-items" dayid="'.$v1->dayarr.'" starttime="'.$v1->starttimearr.'" endtime="'.$v1->endtimearr.'" dissnaparr="'.$v1->dissnaparr.'" snapsendtype="'.$v1->snapsendtypearr.'">'.$v1->dayword.' '.$v1->starttimeword.'-'.$v1->endtimeword.' '.$v1->dissnapword.'<span class="snaplistsubspan">X</span></div>
                                </div>';
                        }
                    }
                }
        ...
        echo json_encode(['datahtml'=>$htmladdsnapArr,'sendtype'=>$sendtype1]);
        die;
    }

    public function delete_user_snap_filter(){
        $user = isset($_REQUEST['user'])?$_REQUEST['user']:'';
        $daycount = isset($_REQUEST['daycount'])?$_REQUEST['daycount']:'';
        $this->CommonController->deleteData('global_snap_user_settings',['user_id'=> $user,'snap_week_days'=> $daycount]);
