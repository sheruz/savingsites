...
<?php 
	...
	public function change_status_type(){
		...
		if($from=='snap'){
			if($status==1){
            	echo '<a href="javascript:void(0);" onclick="emailnoticepopup('.$user_id.','.$createdby_id.','.$zone_id.','.$type.'","",'.$adid.');return false;"><img src="'.$dynamiImageUrl.'/images/turn-on.png" style="width:100%" class="for-img-shedow" /></a>';
            }else {
            	echo '<a href="javascript:void(0);" onclick="emailnoticepopup('.$user_id.','.$createdby_id.','.$zone_id.',"'.$type.'","",'.$adid.');return false;"><img src="'.$dynamiImageUrl.'/images/turn-off.png" style="width:100%" class="for-img-shadow"></a>';
            }
		}else if($from=='newsletter'){
			$this->ad->change_status_type($createdby_id, $user_id, $status, $zone_id,$from);
			if($status==1){
				echo '<a href="javascript:void(0)" onclick="stop_accepting_email_notices("'.$createdby_id.'","'.$adid.'","'.$zone_id.'","'.$user_id.'","newsletter");return false;" class="snapbutton emailIcon"><div class="offerSmallTxt">Subscribed to</div> <div class="offerBigTxt">Newsletter</div></a>';
			...
			if($status){
				echo '<li><a  href="javascript:void(0)" onclick="stop_accepting_email_notices("'.$createdby_id.'","'.$adid.'","'.$zone_id.'","'.$user_id.'","ad_favourites");return false;"><span>Remove<strong>Favorites</strong></span></a></li>';
			} else {
				echo '<li><a  href="javascript:void(0)" onclick="accept_email_notice("'.$createdby_id.'","'.$adid.'","'.$zone_id.'","'.$user_id.'","ad_favourites");return false;"><span>Add to<strong>Favorites</strong></span></a></li>';
			}
		}
	}

	public function approved_business($status=0){ 
		$bus_id = $user_id = $zone = $ad_id = '';
		...
		if($from_where=='snap'){
			if($status) { 
				echo '<a href="#" onclick="stop_accepting_email_notices("'.$bus_id.'","'.$ad_id.'","'.$zone.'","'.$user_id.'","snap");return false;"><img src="'.base_url("assets/stylesheets/images/turn-off.png").'"  class="for-img-shedow" /></a>';
			} else {
				echo '<a href="#" onclick="accept_email_notice("'.$bus_id.'","'.$ad_id.'","'.$zone.'","'.$user_id.'","snap");return false;"><img src="'.base_url("assets/stylesheets/images/turn-on.png").'" class="for-img-shedow" /></a>';
			}
		
		}else if($from_where=='newsletter'){
			if($status){
				echo '<a href="javascript:void(0)" onclick="stop_accepting_email_notices("'.$bus_id.'","'.$ad_id.'","'.$zone.'","'.$user_id.'");return false;" class="adbutton adbutton1 emailIcon">News Letter Activated</a>';
			} else {
				echo '<a href="javascript:void(0)" onclick="accept_email_notice("'.$bus_id.'","'.$ad_id.'","'.$zone.'","'.$user_id.'","newsletter");return false;" class="adbutton adbutton1 emailIcon">Get Our News Letter</a>';
			}
		
		}else if($from_where=='ad_favourites'){
			if($status){
				echo '<a href="javascript:void(0)" class="adbutton favIcon"  onclick="stop_accepting_email_notices("'.$bus_id.'","'.$ad_id.'","'.$zone.'","'.$user_id.'","ad_favourites");return false;">';
				echo '<div class="offerSmallTxt">Remove</div><div class="offerBigTxt">Favorites</div></a>';
			}else{
				echo '<a href="javascript:void(0)" class="adbutton favIcon"  onclick="accept_email_notice("'.$bus_id.'","'.$ad_id.'","'.$zone.'","'.$user_id.'"                     ,"ad_favourites");return false;">';
				echo '<div class="offerSmallTxt">Add to</div><div class="offerBigTxt">Favorites</div></a>';
			}
		}
	}

    public function zone_locator($zone=false){
		if($zone!=''){
			$data=array();
			$data['zone']=$zone;
			$email=$_REQUEST['email'];
    ...
    public function check_zone_locator($zone=false){
		if(isset($_COOKIE['email']) && !empty($_COOKIE['email'])){
			$sql="select id from users where email='".$_COOKIE['email']."'";
			$query = $this->db->query($sql);
			...
			if($query->num_rows()>=1){
				...
				if($query->num_rows()<1){
					echo '<div class="for-two-btn-main" id=""><a href="#" onclick="set_favorite_zone('.$zone.','.$_COOKIE['email'].');">Zone Locator</a></div><div class="for-go-btn-main"><a href="<?=base_url("welcome/index/" . $zone)?>">Go</a></div>';
				}else{
					echo '<div class="for-go-btn-main"><a href="'.base_url("welcome/index/" . $zone).'">Go</a></div>';
				}
			}
		}else{
			echo '<div class="for-two-btn-main" id=""><a href="#" onclick="openVoteBox("favorite");">Zone Locator</a></div><div class="for-go-btn-main"><a href="'.base_url("welcome/index/" . $zone).'">Go</a></div>';
		}
	}

    

    public function find_state_zone($zone=false){
		if(isset($_COOKIE['email']) && !empty($_COOKIE['email'])){
			$sql="select id from users where email='".$_COOKIE['email']."'";
			$query = $this->db->query($sql);
			if($query->num_rows()>=1){
				...
				$query = $this->db->query($sql);
				if($query->num_rows()<1){
					echo '<div class="five-two-btn-main" id=""><a href="#" onclick="set_favorite_zone('.$zone.','.$_COOKIE['email'].');">Zone Locator</a></div><div class="seven-go-btn-main"><a href="'.base_url("welcome/index/" . $zone).'">Go</a></div>';
				}else{
					echo '<div class="seven-go-btn-main"><a href="'.base_url("welcome/index/" . $zone).'">Go</a></div>';
				}
			}
		}else{
			echo '<div class="five-two-btn-main" id=""><a href="#" onclick="openVoteBox("favorite");">Zone Locator</a></div><div class="seven-go-btn-main"><a href="'.base_url("welcome/index/" . $zone).'">Go</a></div>';
		}
	}
	
	public function presentation_demand()//calling from new_page
...
function publisher(){
  ...
  public function getcontactdetail(){
  	...
  	$sql_option="select users.* from users INNER JOIN sales_zone ON users.id=sales_zone.sales_rep_id where sales_zone.id = ".$zoneid;
	$dataArr = $this->CommonController->SelectRawquery($sql_option,'resultArray'); 
	echo json_encode(['msg'=>'data get','type'=>'success', 'email'=>$dataArr[0]['email'],'firstname'=>$dataArr[0]['first_name'],'lastname'=>$dataArr[0]['last_name']]);
	die;
  }
    ...
    public function logintozone(){
		...
		$sqlsub="select * from zone_users where username LIKE '".trim($username)."' AND password LIKE '".trim($password)."'";
		$datasubArr = $this->CommonController->SelectRawquery($sqlsub,'resultArray');
		if(count($datasubArr) <= 0){
			echo json_encode(['msg'=>'Sub User Username and Password Not Exists','type'=>'warning']);
			die;
		}
		if($datasubArr[0]['type'] == 'zonedashboard'){
			$sql_option="select users.* from users INNER JOIN sales_zone ON users.id=sales_zone.sales_rep_id where sales_zone.id = ".$datasubArr[0]['zoneid'];
			$dataArr = $this->CommonController->SelectRawquery($sql_option,'resultArray');
			$userid = isset($dataArr[0]['id'])?$dataArr[0]['id']:'';
			if($userid <= 0){
				echo json_encode(['msg'=>'Sub User Zone Owner Not Exists','type'=>'warning']);
				die;	
			}
			if($userid > 0){
		...
		}else if($datasubArr[0]['type'] == 'business'){
			$userid = isset($datasubArr[0]['zoneowner'])?$datasubArr[0]['zoneowner']:''; 
		}
    	if($userid > 0){
			$sql="select * from business where business_owner_id='".$userid."'";
    			$r = $this->CommonController->SelectRawquery($sql,'row');
    			echo json_encode(['businessid'=>$r->id,'typesub'=>'business','subuser'=> $username]);
    			die;
			}
		}
	}
