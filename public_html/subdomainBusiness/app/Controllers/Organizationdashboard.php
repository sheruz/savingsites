...
class Organizationdashboard extends BaseController{
	...
	public function organizationdetail($org_id = '',$page = 'organizationdetail'){ 
		...
		$pagesidebar = isset($page)?$page:'organizationdetail';
		$zone_name ='';
		$zoneid = '';
		$org_owner_id = '';
		...
		$zone= $this->Zone_model->get_zone($fromzoneid);
	   	...
	   	$username = $organization_owner_detail['username'];
		...
		$sql_get_categories = "SELECT a.state as state_id,a.primarycity, b.zone_id,b.zip_code,c.name as state_name FROM zipcode as a LEFT JOIN zip_code_zone as b ON a.zip = b.zip_code LEFT JOIN states as c ON a.state = c.code LIMIT 20";
      	$get_all_states = $this->CommonController->SelectRawquery($sql_get_categories, 'result');
 	...
 	return view('organizationdashboard',array('org_id'=>$org_id,'pagesidebar'=>$pagesidebar,'zone_name'=> $zone_name,'zoneid'=> $zoneid,'org_owner_id'=> $org_owner_id,'organization_status'=> $organization_status,'from_zoneid'=> $from_zoneid,'group_id'=> $group_id,'username'=> $username,'user_id'=> $user_id,'refer_code'=> $refer_code,'zone_id'=> $zone_id,'zone_owner'=> $zone_owner,'via'=> $via,'login_type'=> $login_type,'organizationid'=> $organizationid,'referralCode'=> $referralCode,'organization_owner_detail'=> $organization_owner_detail,'state_list'=> $state_list,'common'=> $common,'webniar_exist'=> $webniar_exist,'fromzoneid'=> $fromzoneid,'getall_category'=> $getall_category,'countallannouncements'=> $countallannouncements,'limit'=> $limit,'right_container'=> $right_container,'organization_id'=> $org_id,'pboo_credit_payment_data'=> $pboo_credit_payment_data,'x'=> $x,'details_payment_list'=> $details_payment_list,'jotform_category_data'=> $jotform_category_data,'get_all_states'=> $get_all_states,'uid'=> $uid,'totalsumfavorg'=> $totalsumfavorg,'orgyearsum'=> $orgyearsum,'announcement_list'=> $announcement_list,'jotform_category_data'=> $jotform_category_data,'details_payment_list'=> $details_payment_list,'pboo_credit_payment_data'=> $pboo_credit_payment_data,'pboo_credit_account_id'=> $pboo_credit_account_id,'buttonhtml' =>$buttonhtml,'amazonurl'=>$amazonurl));		
	}
...
// This is  for static data  downlod only  code end
	...
	public function getorgcatimage(){
		$catid = isset($_REQUEST['catid'])?$_REQUEST['catid']:'';
		$org_id = isset($_REQUEST['org_id'])?$_REQUEST['org_id']:'';
		$sql_get_categories = "SELECT image_name as img FROM organization_photos WHERE org_id=".$org_id." AND cat_id=".$catid."";
      	$getimage = $this->CommonController->SelectRawquery($sql_get_categories, 'row');
      	if($getimage != ''){
      		$img = isset($getimage->img)?$getimage->img:'';
      	}else{
      		$img = '';
      	}
