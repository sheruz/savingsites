        ...
        if(count($snap_filter_arr) > 0){
            foreach ($snap_filter_arr as $k => $v){
                ...
                $dataArr[$eachday][] = [
                    ...
                    'dissnaparr'     => $eachdiscountsnap,
                    'snapsendtypearr'   => $eachsnapsendtype,
                ];
            }
        }else{
        	$this->CommonController->deleteData($table,['user_id'=>$businessuser]);	
        	$msg = ['msg'=>'SNAP Deleted Successfully','type'=>'success'];
        }

        if(count($dataArr) > 0){
            foreach ($dataArr as $dayk2 => $v2) {
                $qry = "SELECT * FROM ".$table." WHERE user_id=".$businessuser." AND snap_week_days=".$dayk2."";
                $exists = $this->CommonController->SelectRawquery($qry,'result');
                $oldsnap_time = isset($exists[0]->snap_time)?$exists[0]->snap_time:'';

                if(count($exists) > 0){
                    $this->CommonController->deleteData($table,['user_id'=>$businessuser,'snap_week_days'=>$dayk2]);
                }
                if($oldsnap_time != ''){
                ...
                $insertArr = array('sendtype'=>$radioStatus,'user_id'=>$businessuser,'bus_id'=>$businessid,'snap_week_days'=>$dayk2,'snap_time'=>$snaptimeimplode,'created_for_zone'=>$zone_id,'status'=>1);
                $this->CommonController->InsertData($table, $insertArr);
                $this->CommonController->updateData('users',['snap_settings'=>1],['id'=>$businessuser]);  
                $msg = ['msg'=>'SNAP Added Successfully','type'=>'success'];
            }
        }
...
// employee registeration form 
  ...
  if (empty($buffer)){
	...
	function emailnoticeinsertdata(){
		...
		isset($_REQUEST['postal_address']) ? $postal_address=$_REQUEST['postal_address'] : $postal_address='';
		isset($_REQUEST['additionalDelivery']) ? $additionalDelivery = $_REQUEST['additionalDelivery'] : $additionalDelivery = '';
		isset($_REQUEST['street_address']) ? $street=$_REQUEST['street_address'] : $street='';
		...
		isset($_REQUEST['telephone']) ? $telephone=$_REQUEST['telephone'] : $telephone='';
		...
		isset($_REQUEST['refer_code']) ? $refer_code=$_REQUEST['refer_code'] : $refer_code='';
		isset($_REQUEST['zone_id']) ? $zone_id=$_REQUEST['zone_id'] : $zone_id='';
		...
		isset($_REQUEST['refferalcode']) ? $refferalcode=$_REQUEST['refferalcode'] : $refferalcode='';
		isset($_REQUEST['usergiftemail']) ? $usergiftemail=$_REQUEST['usergiftemail'] : $usergiftemail='';
		...
		$usernameqry="SELECT a.phone,b.group_id FROM users as a INNER JOIN users_groups as b ON a.id=b.user_id WHERE a.username = '".$username."'";
		...
		$userorgquery="SELECT a.phone,b.group_id FROM users as a INNER JOIN users_groups as b ON a.id=b.user_id WHERE a.phone = '".$cellPhone."' AND b.group_id=10";
		...
		$userorgnoquery="SELECT a.phone,b.group_id FROM users as a INNER JOIN users_groups as b ON a.id=b.user_id WHERE a.email = '".$email."' AND b.group_id=10";
		...
		$additional_data = array('first_name'=>$first_name,'last_name'=>$last_name,'City'=>$city,'phone'=>$cellPhone,'Zip'=>$zipcode,'Address'=>$state."<br/>".$cellphone1."<br/>".$cellphone2,'company'=>'','Zone_ID' => $zone_id,'refer_code' => $refer_code,'referAmount' => $referAmount);
		...
		if($usergiftemail != ''){
			$this->sendgiftdata($usergiftemail,$data['emailnoticeuser']);
		}	 
		...
		if($email){
			...
			$this->CommonController->SendMail('',$email,'New Registration',$message_body);
		}
 	...
 	public function sendgiftdata($email = '',$userId = ''){
 		$tbl_dealqry = "SELECT * FROM tbl_deals_purchased WHERE certreciever='".$email."' AND certificate_verify=0";
		$tbl_dealArr = $this->CommonController->getStoreCache($tbl_dealqry,'row','tbldealdata',3600);
		
		$tbl_dealuserqry = "SELECT * FROM users WHERE id='".$tbl_dealArr->userId."'";
		$tbl_dealuserArr = $this->CommonController->getStoreCache($tbl_dealuserqry,'row','tbldealuserdata',3600);
		$sendemail = $tbl_dealuserArr->email;
		
		if($tbl_dealArr != ''){
			$tbl_deal_arr = ['userId' => $userId,'zoneId' => $tbl_dealArr->zoneId,'payerId' => $tbl_dealArr->payerId,'amountPrchased' => $tbl_dealArr->amountPrchased,'purchasedAt' => $tbl_dealArr->purchasedAt,'busId' => $tbl_dealArr->busId,'dealId' => $tbl_dealArr->dealId,'user_type' => $tbl_dealArr->user_type,'token' => $tbl_dealArr->token,'certificate_verify' => 0,'certreciever' => '','giftsenderemail' => $sendemail
			];
			$this->CommonController->InsertData('tbl_deals_purchased', $tbl_deal_arr);
			$this->CommonController->updateData('tbl_deals_purchased',['certificate_verify'=>1],['certreciever'=>$email,'userId' => $tbl_dealArr->userId]);
		}
		
		$tbl_dealmetaqry = "SELECT * FROM tbl_deals_purchased_meta WHERE certreciever='".$email."' AND certificate_verify=0";
		$tbl_dealmetaArr = $this->CommonController->getStoreCache($tbl_dealmetaqry,'resultArray','tbldealmetauserdata',3600);
		
		if(count($tbl_dealmetaArr) > 0){
			foreach ($tbl_dealmetaArr as $k => $v) {
				$tbl_deal_meta_arr = ['userId' => $userId,'zoneId' => $v['zoneId'],'totalamount' => $v['totalamount'],'discount' => $v['discount'],'amountPurchased' => $v['amountPurchased'],
        			'purchasedAt' => $v['purchasedAt'],'busId' => $v['busId'],'dealId' => $v['dealId'],'token' => $v['token'],'certificate_verify' => 0,'certreciever' => '','giftsenderemail' => $sendemail
        		];
				$this->CommonController->InsertData('tbl_deals_purchased_meta', $tbl_deal_meta_arr);
				$this->CommonController->updateData('tbl_deals_purchased_meta',['certificate_verify'=>1],['certreciever'=>$email,'userId' => $v['userId']]);	
			}
		}
	}

	//  employee regsiteration 
	public function employeenoticeinsertdata(){
	    //print_r($_REQUEST);die('eee');
	 	isset($_REQUEST['first_name']) ? $first_name=$_REQUEST['first_name'] : $first_name='';
		isset($_REQUEST['last_name']) ? $last_name=$_REQUEST['last_name'] : $last_name='';
	    ...
	    $phone = str_replace("-", "", $phone);
 		...
 		$cellPhone = str_replace("-", "", $cellPhone);
 		$emailnotice_phone = str_replace("-", "", $cellPhone);
		/*check phone number exists*/
		$usernameqry="SELECT a.phone,b.group_id FROM users as a INNER JOIN users_groups as b ON a.id=b.user_id WHERE a.username = '".$username."'";
		$usernameexists = $this->CommonController->SelectRawquery($usernameqry,'count');
		if($usernameexists > 0){
		...
		$userorgquery="SELECT a.phone,b.group_id FROM users as a INNER JOIN users_groups as b ON a.id=b.user_id WHERE a.phone = '".$cellPhone."' AND b.group_id=15";
		$userexists = $this->CommonController->SelectRawquery($userorgquery,'count');
		if($userexists > 0){
			echo json_encode(['msg'=>'User Phone Number Already Exists, Please Use Another Phone Number','type'=>'warning']);
			die;
		}
		$userorgnoquery="SELECT a.phone,b.group_id FROM users as a INNER JOIN users_groups as b ON a.id=b.user_id WHERE a.email = '".$email."' AND b.group_id=15";
		$userenoxists = $this->CommonController->SelectRawquery($userorgnoquery,'count');
		if($userenoxists > 0){
			echo json_encode(['msg'=>'User Email Already Exists, PLease Use Another Email Address','type'=>'warning']);
			die;
		...
		/*check phone number exists*/
		$zone_id = $this->CommonController->redirectToZone();
		$globalSnapSendType = 3;
		$additional_data = array('first_name'=>$first_name,'last_name'=>$last_name,'City'=>$city,'phone'=>$emailnotice_phone,'Zip'=>$zipcode,'Address'=>$state."<br/>".$cellphone1."<br/>".$cellphone2,'company'=>'','Zone_ID' => $zone_id);
		
		$data_peekaboo_bidder=array('fName'=>$first_name,'lName'=>$last_name,'email'=>$email,'city_name'=>$city,'state_name'=>$state,'phone'=>$emailnotice_phone,'user_name'=>$username,'password'=>sha1($password),'register_date'=>time(),'balance'=>20,'activated'=>'yes','activation_number'=>str_shuffle('dGhKYW4wNlR1ZUphbjIwMTYyZHlqb3UxNjAxMDUwNjAxMDA'),'member_type'=>1,'fromreferrer' => $refferalcode);
		...
		if($refferalcode){
			$sql = "SELECT organization.id FROM `users` LEFT JOIN users_groups ON users_groups.user_id = users.id LEFT JOIN organization ON organization.userid = users.id WHERE users_groups.group_id = 8 AND users.referral_code = '".$refferalcode."'";
			$organization_id = $this->CommonController->SelectRawquery($sql,'result');
			$id = $organization_id[0]->id;
	    	if($id){
		    	$data_org1=array('userid' => $data['emailnoticeuser'],'zoneid'=>$zone_id,'org' => $id);
		    	$id = $insert_id = $this->CommonController->InsertData('tbl_selected_nonorg', $data_org1); 
		...
		if($short_url!='')
			...
			$zone_url = 'http://'.$get_zone_details['seo_zone_name'].'/savingssites.com';

		if(!empty($emailvalidation)==1){ 
			$subject = 'Thank you for signing up with '.$zone_name.'';
			$message_body =	'<div style="color:#000;background: #fff;width: 70%;margin: 50px auto;padding: 11px;border: 1px solid #ccc;font-size: 14px;line-height: 26px;box-shadow: 0px 1px 11px rgb(111 110 122 / 52%);"><div style="text-align: center;background: #f2f2f2;padding: 17px;margin-bottom: 20px"><img style="width: 240px;" src="https://cdn.savingssites.com/logo-green.png"/></div>
						...
						<div style="background-color:#FFF; margin-top:10px; margin-bottom:10px; min-height:300px; padding:15px;">
							...
							<h3><p style="text-align:left; display:block; color:#333;">Telephone: '.$emailnotice_phone.'</p></h3>
							<h3><p style="text-align:left; display:block; color:#333;">Email: '.$email.'</p></h3>
							<h3><p style="text-align:left; display:block; color:#333;">Best Regards,<br />'.$zone_name.'<br/></h3>
						</div>
						<div style="background-color:#fff; height:60px;"></div>
					</div>
				</div>
			</body>';
			$this->CommonController->SendMail($fromemail,$email,$subject,$message_body);
		}
		echo json_encode(array('user_id'=>$data['emailnoticeuser'],'createdby_id'=>$createdby_id,'zone_id'=>$zone_id,'type'=>$type,'offer_type'=>$offer_type,'msg'=>'Employee Registered Successfully','type'=>'success'));
		 //echo json_encode(['msg'=>'Employee Registered Successfully','type'=>'success']);
		die; 
	}
...
//  visitor regsiteration 
	public function visitornoticeinsertdata(){
	 	isset($_REQUEST['first_name']) ? $first_name=$_REQUEST['first_name'] : $first_name='';
		isset($_REQUEST['last_name']) ? $last_name=$_REQUEST['last_name'] : $last_name='';
		isset($_REQUEST['street']) ? $street=$_REQUEST['street'] : $street='';
		isset($_REQUEST['city']) ? $city=$_REQUEST['city'] : $city='';
		...
		$phone = str_replace("-", "", $phone);
 		$cellphone1 = str_replace("-", "", $cellphone1);
 		$cellphone2 = str_replace("-", "", $cellphone2);
 		$cell_phone = str_replace("-", "", $cellPhone);
 		$emailnotice_phone = str_replace("-", "", $cellPhone);
		...
		$userorgquery="SELECT a.phone,b.group_id FROM users as a INNER JOIN users_groups as b ON a.id=b.user_id WHERE a.phone = '".$cellPhone."' AND b.group_id=7";
		$userexists = $this->CommonController->SelectRawquery($userorgquery,'count');
		if($userexists > 0){
			echo json_encode(['msg'=>'Visitor Phone Number Already Exists, Please Use Another Phone Number','type'=>'warning']);
			die;
		}
		$userorgnoquery="SELECT a.phone,b.group_id FROM users as a INNER JOIN users_groups as b ON a.id=b.user_id WHERE a.email = '".$email."' AND b.group_id=7";
		$userenoxists = $this->CommonController->SelectRawquery($userorgnoquery,'count');
		if($userenoxists > 0){
			echo json_encode(['msg'=>'Visitor Email Already Exists, PLease Use Another Email Address','type'=>'warning']);
			die;
		}
		/*check phone number exists*/
 			...
 			$message_body =	'<body style="background-color:#FFF; font-family:Arial, Helvetica, sans-serif;">
				<div style="width:960px; margin:0 auto !important;">
					<div style="background-color:#f2f2f2; border-radius:4px; width:650px; margin:5px auto; padding:15px;">
						...
						<div style="clear:both"></div>
						<div style="background-color:#FFF; margin-top:10px; margin-bottom:10px; min-height:300px; padding:15px;">
							<h2 style="text-align:left;">Dear '.$first_name." ".$last_name.',<br>'.'</h2><h3><p style="text-align:left; display:block; color:#333;">Thank you for signing up with '.$zone_name.'.<br>Below are your registered details:</h3><h3><p style="text-align:left; display:block; color:#333;">Username: '.$username.' </p></h3><h3><p style="text-align:left; display:block; color:#333;">First Name: '.$first_name.'	</p></h3><h3><p style="text-align:left; display:block; color:#333;">Last Name: '.$last_name.'</p></h3><h3><p style="text-align:left; display:block; color:#333;">Telephone: '.$emailnotice_phone.'</p></h3><h3><p style="text-align:left; display:block; color:#333;">Email: '.$email.'</p></h3><h3><p style="text-align:left; display:block; color:#333;">Best Regards,<br />'.$zone_name.'</h3>
						</div>
						<div style="background-color:#999; height:60px;"></div>
					</div>
				</div>
			</body>';
			$this->CommonController->sendemailssl($fromemail,$email,$template_subject,$message_body);
		}
		echo json_encode(array('user_id'=>$data['emailnoticeuser'],'createdby_id'=>$createdby_id,'zone_id'=>$zone_id,'type'=>$type,'offer_type'=>$offer_type,'msg'=>'Visitor Registered Successfully','type'=>'success'));
		die;
	}
