...
class Csv extends Model
{
    ...
    public function I_Business($bname = '',$addressid = '',$firstname = '', $lastname = '', $email = '', $website = '', $about = '',$logourl = '',$vicinity = '', $hours_of_operation ='',$history =''){
        if(trim($bname) == ""){
    ...
    public function I_User($username,$email,$firstname, $lastname,$password,$salt,$pass,$zone_id){
    ...
    public function I_Ad($image,$businessid,$starterad,$categoryid,$subcategoryid,$deal_title= ''){
        // $url = $image;
        // $parsedUrl = parse_url($url);
        // $pathInfo = pathinfo($parsedUrl['path']);
        // $saveImagePath = $_SERVER['DOCUMENT_ROOT']."/assets/SavingsUpload/Business/".$businessid;
    ...
    public function I_AdToCategory_Subcategory($adid,$zoneid,$subcatid){ 
        $catid='1'; 
        ...
        if(count($explode) > 0){
            foreach ($explode as $sub) {
                ...
                $id = $this->CommonController->InsertData('business_sponsor_order_cat', $ad_category_subcategorydata);
            }
        } 
        return $id;
    }
