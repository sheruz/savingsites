...
inner join users as t3 on t2.sales_rep_id = t3.id where t1.businessid = ".$businessid;
    ...
    function check_username($username=false){ 
	 ...
	 function add_business_check_username($username=false){ 
	...
	function check_email($email=false, $type=0){ //var_dump($username);
	...
	function get_user_details($uid=false,$key = ''){
		$common = new CommonController();
		$sql="SELECT * FROM users WHERE id=".$uid;
		$result = $common->getStoreCache($sql,'resultArray',$key,3600);
		foreach ($result as $k => $v) {
			$alternatenoquery="SELECT `phone` FROM users_multiple_phonesno WHERE user_id=".$v['id'];
			$result[$k]['alternateno'] = $common->SelectRawquery($alternatenoquery,'resultArray');
			$comnamequery="SELECT `name` FROM business WHERE business_owner_id=".$v['id'];
			$busnameArr = $common->SelectRawquery($comnamequery,'row'); 
			$busname = isset($busnameArr->name)?$busnameArr->name:'';
			$result[$k]['company'] = $busname;

			$memberquery="SELECT * FROM tbl_member WHERE user_name='".$v['username']."'";
			$memberArr = $common->SelectRawquery($memberquery,'row'); 
			$result[$k]['address1'] = isset($memberArr->address1)?$memberArr->address1:'';
			$result[$k]['city_name'] = isset($memberArr->city_name)?$memberArr->city_name:'';
			$result[$k]['state_name'] = isset($memberArr->state_name)?$memberArr->state_name:'';
			$result[$k]['post_code'] = isset($memberArr->post_code)?$memberArr->post_code:'';
		}
		return $result[0];
	}
...
public function get_profile_update($id,$fn,$ln,$phone, $carrier,$email,$alternatephone = [],$res_gen = '',$res_address = '',$res_state = '',$res_city = '',$res_zip = ''){
	$common = new CommonController();
	$sr = 0;
	$userdata = array('first_name' => $fn, 'last_name'=>$ln , 'phone'=>$phone, 'carrier'=>$carrier,'email'=>$email,'gender'=>$res_gen,'Address'=>$res_address,'State_Code'=>$res_state,'City'=>$res_city,'Zip'=>$res_zip);
	$common->updateData('users',$userdata,['id' =>$id]);

	$tbl_member_id = $common->getsinglecolumndata('users','username','id',$id);
	$tbl_member_data = array('fName' => $fn, 'lName'=>$ln , 'phone'=>$phone,'email'=>$email,'gender'=>$res_gen,'address1'=>$res_address,'state_name'=>$res_state,'city_name'=>$res_city,'post_code'=>$res_zip);
	$common->updateData('tbl_member',$tbl_member_data,['user_name' =>$tbl_member_id]);
	if(count($alternatephone) > 0){
		$data['id'] = $id;    

		$builder = $this->db->table('users_multiple_phonesno');
		$builder->where('user_id', $id);
		$builder->delete();  
		 // $this->load->database();
		// $this->db->delete('users_multiple_phonesno', array('user_id' => $id));	
		foreach ($alternatephone as $v) {
			$sr++;
			$alphone = str_replace("-", "", $v);
			$newdata = array(
				'user_id' => $id,
				'counter' => $sr,
				'phone' => $alphone
			);
			$vb = $this->db->table('users_multiple_phonesno');
			$vb->insert($newdata);
		// $this->db->insert('users_multiple_phonesno', $newdata);
		}
	}
	echo json_encode(['type' => 'success','msg' => 'Successfully Updated.']);
}
