 ...
 foreach ($addlist as  $value) {
		...
		foreach ($auction_result_setting as $key =>  $auction_result_setting) {
			...
			if($auction_result_setting['card_img']){
				...
				<tbody><tr>
				<td style="width:100%;padding-right:0px;padding-left:0px;padding-top:15px;">
				<div align="left" style="line-height:10px"><img src="https://cdn.savingssites.com/support_orgs.jpg" style="display: block; height: auto; border: 0; width: 235px; max-width: 100%;" width="235"></div>
				</td>
				</tr>
				</tbody></table>';	
			}
 ...
 $htmlAds .= '</tbody></table>';
  ...
  public function searchBusinessads(){
	...
	public function zonedetail($page='',$fromzoneid=0,$sess_destroy=0){
	  ...
	  $businessArr = $pbcashcert = $categories = $keyaws = $EmailformatingSaved = $subcatnew = $all_sponsored_business_details = $selected_display_categories = $display_categories =  $display_categories = $adsarr = $get_paypal_info = $purchasedealArr = $userpassArr = $savedorganization = $businessrankbid = $totalsumfavorg = $monthArr = $datasubuserArr1 = [];
	  $login_type = $login_id = $refer_code = $user_id = $twiliossid = $twilioauthtoken = $twiliophoneno = $twiliodbid = $twiliodbactive = $twilioaudiofle = '';
		$nonfavper = 10;
	...
	public function getMyZoneBusiness($zoneid = 0,$lowerlimit = 1,$upperlimit = 10){
	   ...
	   $joinArr[] = ['table' => 'address as c','link' => 'c.id=a.addressid','type' => 'left'];
   	   ...
   	   $select = 'a.id,a.name,a.contactfirstname,a.contactlastname,f.phone,c.zip_code,a.comm_method';
...
################################################  Zone Business Part Start #################################################################
	...
	*/
	...
	public function create_business(){
		...
		if($zone_id != -1){
			$sql_get_zone_owner_email = "SELECT b.id , b.name , a.email , a.first_name , a.last_name FROM users a , sales_zone b WHERE a.id = b.sales_rep_id AND b.id=".$zone_id;
		...
		$message_body.= '<h3><p style="text-align:left; display:block; color:#333;">Business Credentials Info - </p></h3>
			...
			<h3><p style="text-align:left; display:block; color:#333;">Password: '.$biz_password.'</p></h3>
			...
			<h3><p style="text-align:left; display:block; color:#333;"><a href="'.base_url().'">savingssites.com</a></p></h3></div>
...
################################################  New Organization Start ####################################################################################	
	...
	*/
	...
	public function organization_registration($id= -1,$organizationname='',$accounttype = '',$email = '',$username = '',$zipcode= '',$state = '',$city='',$firstname='',$lastname='',$phone='',$address='',$password='',$salt='',$pass='',$selzone = ''){
		$group_arr = array();
		...
		echo "<pre>";print_r($organization_owner_id);die;
		$this->ionAuth->change_user_group($organization_owner_id);

		$data_org=array('name' => $org_name,'type'=>$org_type,'zoneid' => $zone_id,'userid'=>$organization_owner_id,'announcement_display'=>0,'approval'=>1); 
		...
		// + Start Organization create seller account on peekaboo 
		if($id>0 && $id!=''){
			$data_peekaboo=array(
				'fName'=>$fname,
				'lName'=>$lname,
		...
		// - Organization create seller account on peekaboo
		...
		if($id>0){
			$message = '<div class="succ_messs_form">Registration Successful.</div>';
			$message_body="<div style='border:1px solid #900; padding:5px;'><font size='4'>Dear ".$org_name.",<br /><br />
			...
			if($org_email!=''){
				$this->email->setTo($org_email);
				$this->email->send();
				$to[]=$org_email;
			}
			
			if($selZone != -1){ 		
				$message = '<div class="succ_messs_form"><span style="text-align:center; margin-left: 139px;">Thank You For Registering!</span><br />
					<span style="text-align:center; margin-left: 110px;">We Will Be Contacting You Shortly.</span></div>';
				$message_body="<div style='border:1px solid #900; padding:5px;'><font size='4'>Hi, ".$ZO_fname."<br /><br />
					...
					$fromemail=$this->myconfig->adminEmailId;
        			$template_subject="New Organization Registration";
        			$this->email->setFrom($this->myconfig->fromemail);
				...
				$message = '<div class="succ_messs_form"><span style="text-align:center; margin-left: 139px;">Thank You For Registering!</span><br />
					<span style="text-align:center; margin-left: 110px;">We Will Be Contacting You Shortly.</span></div>';
				$message_body="<div style='border:1px solid #900; padding:5px;'<font size='4'>Hi, ".$zip_Zo_fname."<br /><br />
	...
	* This function is updating the organization details. 
	...
	* <li><b>update_user_info_for_organization</b> (Model Name: ion_auth)</li>

	* <li><b>get_organization_for_zone</b> (Model Name: org_model)</li>

	* <li><b>get_zoneinformation</b> (Model Name: announcements)</li>

	* </ol>

	* <br>

	* view page: <b>views/dashboards/zone_parts/organization_display</b>
