    ...
    <input type="hidden" id="businessid" name="businessid" value="<?php echo $businessid;?>"/>
      <div class="row" style=" margin-bottom: 80px;">
         <div class="top-title">
            ...
            <p>Below are the deals offered by all the businesses. Each deal has a limited time to be featured on our deal page. Once a deal expires, it is removed and replaced with a business ad. You can extend the deal's duration by 30 days using the 'Re-schedule' button. Use the 'Re-generate Deal' button when the deal quantity reaches zero.</p>
         ...
         <table class="display responsive-table datatable-all" cellspacing="0" width="100%" id="managecat">
            ...
            <tbody>
               <?php foreach ($businessArr as $vdataarray) { foreach($vdataarray as $value){ ?>
               <tr>
                  ...
                  <td>
                     <span class="editdeal cus-solid-prpl" busname="<?= $value['businessname']; ?>" busid="<?= $value['businessid']; ?>" zone_id="<?= $zone_id; ?>" productid="<?= $value['product_id'] ?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</span>
                     <span href="#" proid='<?php echo $value['product_id'] ?>' class="reschedulebtn cus-solid-blue"><i class="fa fa-calendar" aria-hidden="true"></i> Re-schedule</span>
                    <button type="button" class="btn btn-info" id="matchupdate" produtid="<?= $value['product_id'] ?>">Re-generate Deal</button>
                  </td>
               </tr>
               <?php   }   } ?>
            </tbody>
