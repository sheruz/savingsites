...
<?php } ?>
<div class="container_tab_header">Organization Payment Certification</div>
<div class="container_tab_content ui-widget-content">
<div id="table_contet">
<?php if(count($organization_list) > 0){?>
  ...
  <thead id="showhead" class="headerclass">
    <tr>
      ...
      <th width="15%">Ogranization Name</th>
      <th width="15%">Profit Amount</th>
      <th width="10%">Payment Purpose</th>
      <th width="10%">Percentage Amount to Pay</th>
      <th width="10%">Amount</th>
      <th width="15%">Action</th>
    </tr>
  </thead>
  <tbody>
  <?php //var_dump($organization_list); ?>
  <?php foreach ($organization_list as $value) {
  	$shared_profit_amount = (8/100)*$value['buy_price_decrease_by'];
    $organization_payment = (12.5/100)*$shared_profit_amount;
   ?>
  	<tr>
      <td><?= $value['payment_id']; ?></td>
  		<td><?= $value['organization_id']; ?></td>
  		<td><?= $value['name'] ?></td>
      <td><?= $shared_profit_amount; ?></td>
  		<td>Share certification profit to organization</td>
      <td><input type="number" step="0.05" value="12.5" class="share_percentage" data-amount ="<?= $shared_profit_amount; ?>" onChange="dynamic_amount_calculation(this);"></td>
      <td class="amount_div"><?= $organization_payment; ?></td>
  		<td>
      <?php if($value['status']){?>
      <button type="btn btn-success" class="view_business_payment_for_certificate" data-receiver-id="<?= $value['organization_id'] ?>" data-payer-id="<?= $zone_id; ?>" data-amount="<?= $organization_payment; ?>" data-payment-id ="<?= $value['payment_id']; ?>">Paid</button>
      <?php } else{ ?>
      <button type="btn btn-success" class="business_payment_for_certificate" data-receiver-id="<?= $value['organization_id'] ?>" data-payer-id="<?= $zone_id; ?>" data-amount="<?= $organization_payment; ?>" data-payment-id ="<?= $value['payment_id']; ?>">Pay Now</button>
      <?php } ?>
      </td>
  	</tr>
 <?php } ?>
...
<?php } else{ ?>
  <div class="container_tab_header" id="not_found">No data Found</div>
<?php } ?>
</div>
</div>
</div>
<script type="text/javascript">
    ...
    var shared_profitd_amount = "<?= $shared_profit_amount; ?>";
      ...
      //alert(payment_id);
			...
			window.location.href="<?= base_url('Zonedashboard/organization_certification_payment') ?>/"+payer_id+"/"+receiver_id+"/"+amount+"/"+payment_id;

		});
	});
  function dynamic_amount_calculation(element){
    var current_percentage = $(element).val();
    var profit_amount = $(element).attr('data-amount');
    var amount = (current_percentage/100)*profit_amount;
        amount = amount.toFixed(amount);
    $(element).parent().next('.amount_div').text(amount);
    $(element).parent().next().next().find('.business_payment_for_certificate').attr('data-amount',amount);
    //alert(amount);
