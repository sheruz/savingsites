...
if($data){
	...
	<li class='draggable_div' style='position:relative;'    businessid ="<?php echo $ordered_business['businessId'];  ?>"  data-adid="<?php echo $ordered_business['sort_order'];  ?>" id="<?php echo $ordered_business['res_id'];  ?>"><p><?php echo $ordered_business['restaurant_name'] ?></p> <button class='counter' title='sponsor Order' style='top: 0px; position:absolute; right: 0px;  padding: 3px 7px; border-radius: 15px;'><?php echo $i; ?></button>
     <input type="hidden" name="zoneid" class="zoneid" value="<?php echo $_GET['zoneId'] ?>">
	</li>
...
$('#sortable').sortable({
	...
	update: function(event, ui){
		...
		$.post("<?=base_url('Zonedashboard/resta_business_reorder')?>/",data,function(){
