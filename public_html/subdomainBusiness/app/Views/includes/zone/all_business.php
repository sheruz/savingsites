...
<?php } ?>
  ...
  <div class="container_tab_header" style="position:relative;">
    	...
    	<div class="bv_filter_querry" style="padding:12px 0;"><span class="container_tab_header">Filtered Query Results</span><input type="hidden" id="exportpattern" value="" /></div>
      ...
      <div id="tabs-1">
        ...
        <p>Search your business directly by entering the name inside "Direct search by business name" text box and click on the "Search" option. Or search businesses alphabetically by selecting the desired alphabet and then click on the "Search" option.</p>
          ...
          <!-- Search Part Start -->
            ...
            <input type="text" id="text_bus_search" name="text_bus_search" placeholder="Search Name ..."  style="width:260px;" />

             <select class="categorylist">  

              </select>

          	<button class="btn-sm  btn_bus_search_by_names"   type="button">Search</button>
...
<div style="display: none; position: fixed;top: 0;left: 0;right: 0;bottom: 0;background-color: rgba(0, 0, 0, .5);z-index: 1099;" class="page-loader">
 
     <img style="position: absolute;left: 50%;top: 50%;margin-left: -16px;margin-top: -16px;" src="https://cdn.savingssites.com/loading.gif">
...
$("#text_bus_search").val(' ');
    if($(this).val() == 'category'){
          ...
          $("#text_bus_search").val('');
          $("#text_bus_search").css('display','none');
          $(".categorylist").css('display','inline-block');
          
          // ajax list
          $.ajax({
            url: "<?=base_url('/Zonedashboard/listofcategory' )?>",
            type: "POST",    
          ...
          // ajax list
          $.ajax({
            url: "<?= base_url('/Zonedashboard/listofsubcategory') ?>",
            type: "POST", 
            beforeSend: function( xhr ) {
                $('.page-loader').show();
...
$("#text_bus_search").css('display','inline-block');
...
$(document).on('click','.more_non_temp_business',function(){ 
...
if($(this).parent('li').hasClass('active')){
  ...
  var lowerlimit=  $(this).data("lower"); 

  var upperlimit = $(this).data("upper");

  console.log(lowerlimit , upperlimit , 'upperlimit'); 
...
$(document).on('click','#edit_update_non_temp_business',function(){
	...
	var businesszone=$(this).attr('data-businesszone'); //alert(businesszone);return false;
				...
				/*if($('#biz_email').val()== ''){
					 ...
					 $('#error_contactbiz_email').html(txt);

					 $('#error_contactbiz_email').show();
...
$('.btn_bus_search_by_names').click(function(){
   ...
   var typeofbusinesses = $("input[name=typeofbusinesses]:checked").val() ;
    ...
    var search_by_subcat = ''

    var search_by_cat ='';

    if($('.selectoption').val() == 'category'){

        var search_by_cat = $('.categorylist option:selected').val(); 

    }else if($('.selectoption').val() == 'subcategory'){

        var search_by_subcat = $('.categorylist option:selected').val();
    }
    ...
    var bus_search_by_name=$. trim($('#text_bus_search').val()); 


  var data={
           'bus_search_results':'resultbycat',
          'zoneid':$('#zoneid').val(),
          'typeofbusinesses':typeofbusinesses,
          'typeofadds':typeofadds,
          'paymentstatus':paymentstatus,
          ...
          'businessmode':businessmode,
          'bus_search_by_name':bus_search_by_name,
          'bus_search_by_subcategory':search_by_subcat,
          'bus_search_by_cat':search_by_cat           
          };
...
$('.trashlink').live('click',function(e){


  var action_performed = 3;

  var change_business_status=2;
  ...
  var business_type=$('#update_non_temp_business').attr('data-businesstype'); //alert(business_type);

  var business_type_by_category=$('#update_non_temp_business').attr('data-businesstypebycategory'); // alert(business_type_by_category); return false;

  var business_zone=$('#update_non_temp_business').attr('data-businesszone');  

  var businessmode=$('#update_non_temp_business').attr('data-businessmode');//alert(businessmode);return false;

  var typeofadds=$('#update_non_temp_business').attr('data-typeofadds');

  var paymentstatus=$('#update_non_temp_business').attr('data-paymentstatus');

  var activestatus=$('#update_non_temp_business').attr('data-activestatus');

  var typeofbusinesses=$('#update_non_temp_business').attr('data-typeofbusinesses');

  var business_id=$(this).attr('data-busid'); 

  var display_checkbox=$(this).attr('data-busid'); 
  ...
  if(action_performed==3 && action_performed_in_where==0 && business_delete_all_or_specific==1){
	...
	function successdata(result){ 
        ...
        $(".view_non_temp_business").find("tr:gt(0)").remove();
         ...
         $(document).on('blur','#biz_name',function(){
			...
			/*$(document).on('blur','#biz_email',function(){
					...
					if($('#biz_email').val()== ''){
							...
							var txt = '<h5 style="color:#FF0000">Please enter email.</h5>';
